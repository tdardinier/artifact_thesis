...
text \<open>This section corresponds to Section 3.3 of the thesis.\<close>
...
text \<open>The definition of combinable assertions corresponds to Definition 3.3.1 of the thesis.\<close>
...
lemma combinable_instantiate:
  ...
  by (meson assms(1) assms(2) assms(3) assms(4) combinable_def entails_def sat.simps(2) sat.simps(1))
...
proof (rule combinableI_old)
  ...
  proof -
    have "Some ((smult (sinv (sadd p q)) (sadd p q)) \<odot> x) = ?p \<odot> a \<oplus> ?q \<odot> b"
      by (metis \<open>a, s, \<Delta> \<Turnstile> A \<and> b, s, \<Delta> \<Turnstile> A \<and> Some \<sigma> = p \<odot> a \<oplus> q \<odot> b \<and> \<sigma> = sadd p q \<odot> x\<close> double_mult plus_mult)
    then show ?thesis
      by (simp add: one_neutral sinv_inverse smult_comm)
  qed
  moreover have "sadd ?p ?q = one"
    by (metis smult_comm sinv_inverse smult_distrib)
...
proof (rule combinableI_old)
  ...
  proof (rule sat_wand)
    ...
    then obtain baa where "Some baa = b \<oplus> aa"
      using compatible_def sumI by blast
    ultimately have "Some (mult (sadd p q) \<sigma>') = p \<odot> aaa \<oplus> q \<odot> baa"
    proof -
      obtain a1 where "Some a1 = \<sigma> \<oplus> (p \<odot> aa)"
        by (metis \<open>Some (sadd p q \<odot> \<sigma>') = \<sigma> \<oplus> sadd p q \<odot> aa\<close> distrib_mult option.exhaust pre_pcm_class.asso3)
      ...
      proof -
        obtain bc where "q \<odot> b \<oplus> p \<odot> aa = Some bc"
          by (metis \<open>Some a1 = \<sigma> \<oplus> p \<odot> aa\<close>
              \<open>a, s, \<Delta> \<Turnstile> Wand A B \<and> b, s, \<Delta> \<Turnstile> Wand A B \<and> Some \<sigma> = p \<odot> a \<oplus> q \<odot> b \<and> \<sigma> = sadd p q \<odot> x\<close> compatible_def
              local.asso2 sumI)
        ...
        then show ?thesis
          by (metis (full_types) \<open>Some a1 = \<sigma> \<oplus> p \<odot> aa\<close> \<open>Some a2 = p \<odot> a \<oplus> p \<odot> aa\<close> \<open>q \<odot> b \<oplus> p \<odot> aa = Some bc\<close>
              pre_pcm_class.asso1 pre_pcm_class.commutative)
      qed
    ...
    ultimately have "mult (sadd p q) \<sigma>', s, \<Delta> \<Turnstile> Mult (sadd p q) B"
      by (meson assms combinable_def entails_def sat.simps(1) sat.simps(2))
...
proof (rule combinableI_old)
  ...
  then obtain xa xb where "Some xa = p \<odot> aa \<oplus> q \<odot> ba" "Some xb = p \<odot> ab \<oplus> q \<odot> bb"
    by (metis \<open>a, s, \<Delta> \<Turnstile> Star A B \<and> b, s, \<Delta> \<Turnstile> Star A B \<and> Some \<sigma> = p \<odot> a \<oplus> q \<odot> b \<and> \<sigma> = sadd p q \<odot> x\<close> asso2 commutative compatible_iff compatible_multiples one_neutral option.discI option.exhaust_sel compatible_def)
  then have "xa, s, \<Delta> \<Turnstile> Mult (sadd p q) A"
    by (meson \<open>aa, s, \<Delta> \<Turnstile> A\<close> \<open>ba, s, \<Delta> \<Turnstile> A\<close> assms(1) entails_def combinable_def sat.simps(1) sat.simps(2))
  ...
  ultimately show "x, s, \<Delta> \<Turnstile> Star A B"
    by (metis can_divide sat.simps(1) sat.simps(2))
qed
...
proof (rule combinableI)
  ...
  then have "a ## b"
    by (metis compatible_multiples option.discI compatible_def)
  ...
  then show "y, s, \<Delta> \<Turnstile> Exists x A"
    by (metis (mono_tags, opaque_lifting) \<open>a, s(x := v1), \<Delta> \<Turnstile> A\<close> \<open>a, s, \<Delta> \<Turnstile> Exists x A \<and> b, s, \<Delta> \<Turnstile> Exists x A \<and> Some y = p \<odot> a \<oplus> q \<odot> b \<and> sadd p q = one\<close> \<open>b, s(x := v2), \<Delta> \<Turnstile> A\<close> assms(1) combinable_instantiate_one sat.simps(8))
qed
...
lemma combinable_disj_aux_1:
  assumes "pure A \<and> combinable \<Delta> B"
  shows "combinable \<Delta> (Or A B)"
proof (rule combinableI)
  fix a b p q x \<sigma> s
  assume asm0: "a, s, \<Delta> \<Turnstile> Or A B \<and> b, s, \<Delta> \<Turnstile> Or A B \<and> Some x = p \<odot> a \<oplus> q \<odot> b \<and> sadd p q = one"
  show "x, s, \<Delta> \<Turnstile> Or A B"
    apply (cases "a, s, \<Delta> \<Turnstile> A"; cases "b, s, \<Delta> \<Turnstile> A")
    using assms pure_def apply fastforce
    using assms pure_def apply blast
    using assms pure_def apply blast
    using asm0 assms combinable_instantiate_one sat.simps(6) by blast
qed

lemma combinable_disj_aux_2:
...
proof (rule combinableI)
  ...
  show "x, s, \<Delta> \<Turnstile> Or A B"
    apply (cases "a, s, \<Delta> \<Turnstile> A"; cases "b, s, \<Delta> \<Turnstile> A")
    using asm0 assms combinable_instantiate_one sat.simps(6) apply blast
    using asm0 assms pure_def sat.simps(6) apply blast
    using asm0 assms pure_def sat.simps(6) apply blast
    using asm0 assms pure_def sat.simps(6) by blast
qed

lemma combinable_disj_aux_3:
  assumes "pure A \<and> pure B"
  shows "combinable \<Delta> (Or A B)"
proof (rule combinableI)
  fix a b p q x \<sigma> s
  assume asm0: "a, s, \<Delta> \<Turnstile> Or A B \<and> b, s, \<Delta> \<Turnstile> Or A B \<and> Some x = p \<odot> a \<oplus> q \<odot> b \<and> sadd p q = one"
  show "x, s, \<Delta> \<Turnstile> Or A B"
    apply (cases "a, s, \<Delta> \<Turnstile> A"; cases "b, s, \<Delta> \<Turnstile> A")
    using assms pure_def sat.simps(6) apply blast
    using assms pure_def apply blast
    using assms pure_def apply blast
    using assms asm0 pure_def by force
qed

lemma combinable_disjunction:
  assumes "(pure A \<and> pure B) \<or> (pure A \<and> combinable \<Delta> B) \<or> (combinable \<Delta> A \<and> pure B)"
  shows "combinable \<Delta> (Or A B)"
  using assms combinable_disj_aux_1 combinable_disj_aux_2 combinable_disj_aux_3 by blast

lemma pure_disj:
  assumes "pure A"
      and "pure B"
    shows "pure (Or A B)"
  using assms unfolding pure_def
  apply simp by blast
