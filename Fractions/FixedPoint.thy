...
text \<open>This subsection corresponds to Section 3.4 of the thesis.\<close>
...
text \<open>The definition of set-closure properties corresponds to Definition 3.4.4 of the thesis.\<close>
...
proof (rule monotonicI)
  ...
  proof (rule smaller_interpI)
    ...
    then show "x \<in> applies_eq (And A B) \<Delta>' s"
      using asm0 assms(1) assms(2) monotonic_def mem_Collect_eq sat.simps(8) smaller_interp_applies_cons by fastforce
  qed
qed
...
proof (rule non_increasingI)
  ...
  proof (rule smaller_interpI)
    fix s x assume asm1: "x \<in> applies_eq (And A B) \<Delta> s"
    then show "x \<in> applies_eq (And A B) \<Delta>' s"
      using asm0 assms(1) assms(2) non_increasing_def  mem_Collect_eq sat.simps(8) smaller_interp_applies_cons by fastforce
  qed
qed
...
text \<open>The following theorem corresponds to Theorem 3.4.3 of the thesis.\<close>
