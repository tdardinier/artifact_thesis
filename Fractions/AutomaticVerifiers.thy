...
text \<open>This section corresponds to Section 3.5 of the thesis.\<close>
...
text \<open>The following definition corresponds to Figure 3.6 of the thesis.\<close>
...
text \<open>The following theorem corresponds to Theorem 3.5.1 of the thesis.\<close>
...
next
  ...
  proof
    ...
    proof -
      ...
      then show ?Q
        by (metis dot_mult2 entails_def smult_comm)
    qed
    assume ?Q
  ...
  proof
    ...
    proof (rule sat_wand)
      ...
      then have "div_state \<pi> b, s, \<Delta> \<Turnstile> B"
        by (metis (no_types, lifting) Wand.hyps(1) \<open>a, s, \<Delta> \<Turnstile> syn_mult \<pi> A \<and> Some b = \<sigma> \<oplus> a\<close> \<open>div_state \<pi> \<sigma>, s, \<Delta> \<Turnstile> Wand A B\<close> div_state_ok can_divide sat.simps(1) sat.simps(3))
  ...
  proof
    ...
    proof -
      ...
      then obtain "\<sigma>, s, \<Delta> \<Turnstile> syn_mult \<pi> A" "\<sigma>, s, \<Delta> \<Turnstile> syn_mult \<pi> B"
        by auto
      then show ?Q
        by (meson And.hyps(1) And.hyps(2) dot_and2 entails_def sat.simps(7))
    qed
...
text \<open>The following theorem corresponds to Theorem 3.5.3 of the thesis.\<close>
...
text \<open>The following two theorems correspond to the rules shown in Section 3.5.1 of the thesis.\<close>
...
text \<open>The following four theorems correspond to the rules shown in Section 3.5.2 of the thesis.\<close>
...
text \<open>The following theorems correspond to the rule shown in Section 3.5.3 of the thesis.\<close>
...
theorem wf_combine:
  ...
  apply (metis assms(1) assms(2) combinable_def wf_implies_combinable wf_assertion.simps(1) wf_assertion_combinable_lfp)
  by (metis assms(1) assms(2) combinable_def wf_implies_combinable wf_assertion.simps(1) wf_assertion_combinable_gfp)
