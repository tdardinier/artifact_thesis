theory Chapter2_TranslationalVerifiers
  imports
    "SymEx/SymbolicExecEndToEnd"
    "VCG/VCGEndToEnd"
begin
...
mentioned in chapter 2. The theory file is structured using Isabelle sections and subsections,
which match those from the chapter. You can use the "Sidekick" view on the right side of 
...
This document makes use of Isabelle's documentation features, which itself can contain Isabelle
elements (and are checked via Isabelle), to walk you through the formalization of the definitions and 
rules in the chapter. In particular, we use the following Isabelle document elements:
...
We also provide links to files in some cases such as @{file "Viper/Instantiation.thy"}
(which you can also ctrl-click on).
\<close>

section \<open>2.2: Key Ideas\<close>

subsection \<open>2.2.1: A Core Language for SL-Based IVLs\<close>

paragraph \<open>Figure 2.2\<close>
text \<open>The syntax of CoreIVL is defined as the type \<^typ>\<open>('a, 'v, 'c) abs_stmt\<close> 
...
subsection \<open>2.2.3: Operational Semantics and Back-End Verifiers\<close>

context semantics
begin

paragraph \<open>Figure 2.4 a)\<close> 
text\<open> 
\<^const>\<open>red_stmt\<close> defines the operational semantics of CoreIVL, which contains the rules in Figure 2.4 a). 
\<open>\<langle>C, \<omega>\<rangle> \<rightarrow>\<^sub>\<Delta> S\<close> in the thesis is represented by \<^prop>\<open>red_stmt \<Delta> C \<omega> S\<close>.
\<close>

paragraph \<open>Definition 2.2.1\<close>
...
subsection \<open>2.2.4: Axiomatic Semantics\<close>

paragraph \<open>Triple \<open>\<Delta> \<turnstile> [P] C [Q]\<close> and Figure 2.4 b)\<close>
text \<open>
\<^const>\<open>SL_proof\<close> defines the axiomatic semantics of CoreIVL,
which contains the rules in Figure 2.4 b).
\<open>\<Delta> \<turnstile> [P] C [Q]\<close> in the thesis is represented by \<^prop>\<open>\<Delta> \<turnstile> [P] C [Q]\<close> (which is syntactic sugar for 
\<^prop>\<open>SL_proof \<Delta> P C Q\<close>)
\<close>

paragraph \<open>Theorem 2.2.1: Operational-to-Axiomatic Soundness\<close>

text \<open>The following is a more general version of Theorem 2.2.1:\<close>
...
text \<open>From this theorem, we can derive Theorem 2.2.1 as presented in the thesis:\<close>
...
paragraph \<open>Figure 2.5\<close>
text \<open>The CSL Triple \<open>\<Delta> \<turnstile>\<^sub>C\<^sub>S\<^sub>L [P] C [Q]\<close> is formalized as \<^term>\<open>\<Delta> \<turnstile>CSL [P] C [Q]\<close>.
 See the description of Section 2.2.2 above.\<close>

paragraph \<open>Lemma 2.2.2 (Exhale-havoc-inhale pattern)\<close>
text \<open>The following shows lemma 2.2.2 from the thesis.
...
lemma exhale_havoc_inhale:
  ...
  assumes "self_framing P"
      and "self_framing Q"
      and "\<Delta> \<turnstile> [A] abs_stmt.Exhale P;; havoc_list (wrL C);; abs_stmt.Inhale Q [B]"
...
section \<open>2.3: Semantics\<close>

subsection \<open>2.3.1: An Algebra for Separation Logic and Implicit Dynamic Frames\<close>

paragraph \<open>Definition 2.3.1 (IDF algebra) and Figure 2.6\<close>
text \<open>IDF algebras are formalized via the type class \<^class>\<open>sep_algebra\<close>. 
This type class is defined via other type classes. To see all the axioms listed in Figure 2.6, one must
...
contributes and which axioms shown in Figure 2.6 it contains:
...
\<^item> \<^class>\<open>pcm\<close>: Accepts a type and binary operation \<^term>\<open>a \<oplus> b\<close> and contains the first 3 axioms (top row) 
  in Figure 2.6 (note that here, the associativity axiom in Figure 2.6 is split into two axioms)
\<^item> \<^class>\<open>pcm_with_core\<close>: Adds core \<^term>\<open>|x|\<close>, and contains the next 5 axioms in Figure 2.6 (second row
...
\<^item> \<^class>\<open>sep_algebra\<close>: Adds \<^term>\<open>stable\<close> and \<^term>\<open>stabilize\<close>, and contains the last 5 axioms in 
  Figure 2.6 (all axioms in Figure 2.6 that contain \<open>stabilize\<close>)\<close>
...
We prove that \<^typ>\<open>'a virtual_state\<close> forms an IDF algebra in the file
 @{file "Viper/EquiSemAuxLemma.thy"} 
...
paragraph\<open>Definition 2.3.2\<close>
text \<open>Definition 2.3.2:
...
text \<open>The following lemma shows that the definition in the thesis and \<^term>\<open>rel_stable_assertion \<omega> P\<close>
agree for well-formed assertions (i.e., all relevant assertions).\<close>

lemma rel_stable_assertion_same_as_in_thesis:
...
subsection \<open>2.3.2: Operational Semantics\<close>


paragraph \<open>Figure 2.7\<close>
text \<open>
\<^const>\<open>red_stmt\<close> defines the operational semantics of CoreIVL, which contains the rules in Figure 2.7. 
\<open>\<langle>C, \<omega>\<rangle> \<rightarrow>\<^sub>\<Delta> S\<close> in the thesis is represented by \<^prop>\<open>red_stmt \<Delta> C \<omega> S\<close>.
\<close>

subsection \<open>2.3.3: Axiomatic Semantics\<close>

paragraph \<open>Triple \<open>\<Delta> \<turnstile> [P] C [Q]\<close> and Figure 2.8\<close>
text \<open>
\<^const>\<open>SL_proof\<close> defines the axiomatic semantics of CoreIVL, which contains the rules in Figure 2.8.
\<open>\<Delta> \<turnstile> [P] C [Q]\<close> in the thesis is represented by \<^prop>\<open>\<Delta> \<turnstile> [P] C [Q]\<close> (which is syntactic sugar for 
\<^prop>\<open>SL_proof \<Delta> P C Q\<close>)
\<close>

paragraph \<open>Lemma 2.3.1\<close>
text\<open>Lemma 2.3.1 is given by:\<close>

lemma from_operational_to_axiomatic_semantics:
...
paragraph \<open>Theorem 2.3.2: Completeness\<close>
text\<open>Theorem 2.3.2 is given by the following.\<close>
...
text \<open>
Note that in this theorem, as in many theorems below as well, C is required to be well-formed,
i.e., \<^term>\<open>wf_abs_stmt \<Delta> C\<close>, which is ignored in the thesis. This ensures that
...
Also, here \<^term>\<open>\<omega>\<close> is restricted to be well-typed (\<^term>\<open>typed \<Delta> \<omega>\<close>); as discussed in footnote 6,
we ignored typing in the thesis for the sake of presentation, but handle it in this formalization.
\<close>

end

subsection \<open>2.3.4: ViperCore: Instantiating CoreIVL with Viper\<close>
...
\<^item> (1) IDF algebra instance:
    Our ViperCore IDF algebra instance is given by \<^typ>\<open>'a virtual_state\<close> (\<Sigma>_IDF in the thesis),
    which we discuss above as part of Section 2.3.1. The state model of the instantiated CoreIVL
    is \<^typ>\<open>'a equi_state\<close> (equi_states contain one more component of type \<^typ>\<open>'a ag_trace\<close>,
    which is irrelevant for the subset considered in this thesis).
...
\<^item> (4) Proofs that the semantics in (3) are compatible with our framework
   ...
   @{file "Viper/Instantiation.thy"}
\<close>


section \<open>2.4: Back-End Soundness\<close>

subsection \<open>2.4.1: Symbolic Execution\<close>

paragraph \<open>Figure 2.9: Definition of the symbolic execution\<close>
text\<open>The definition of the symbolic execution can be found in
 @{file "SymEx/SymbolicExecDef.thy"} and the
 corresponding automation in @{file "SymEx/SymbolicExecAuto.thy"}
...
paragraph \<open>Theorem 2.4.1\<close>

text\<open>Theorem 2.4.1 corresponds to @{thm sexec_verifies_set}. Note that the formal theorem has some
additional side-conditions that the state and statement are well-typed.\<close>

subsection \<open>2.4.2: Verification Condition Generation\<close>

paragraph \<open>Formalization of Viper's VCG (VCGSem)\<close>

text \<open>The formalization of Viper's VCG (which is not a contribution of this thesis, but is part of 
reference 74 in the thesis) in the thesis is presented via the judgement \<open>\<langle>C, \<sigma>\<^sub>t\<rangle> \<rightarrow>_VCG r\<close>.
...
paragraph \<open>Theorem 2.4.2 (VCGSem)\<close>
text \<open>Theorem 2.4.2 is given by:\<close>
...
text \<open>One difference with the thesis is that here we consider a more general version of Theorem 2.4.2 that 
is parametric in an initial set of states \<^term>\<open>A\<close>. 
Assumption A1 corresponds to \<open>\<not>(\<langle>C, \<sigma>\<^sub>t\<rangle> \<rightarrow>_VCG F)\<close> for all states \<open>\<sigma>\<^sub>t\<close> related to \<^term>\<open>\<omega>\<close> in the thesis
...
well-typed; we omitted these in the thesis for the sake of presentation.
Assumption A5 restricts the Viper statement \<^term>\<open>C\<close> to be in the Viper subset that we consider in the thesis
...
paragraph \<open>Using Theorem 2.4.2 to connect to a Viper method\<close>

text \<open>The VCG back-end operates on Viper methods that contain Viper statements and not on Viper statements.
It is straightforward to use Theorem 2.4.2 in order to obtain a lemma that instead assumes the correctness of 
...
We did not show such a lemma in the thesis, but we have proved such a lemma in Isabelle.
...
of Theorem 2.4.2 and proves assumptions A3 and A4 above for that instantiation.
\<close>
...
section \<open>2.5: Front-End Soundness\<close>

subsection \<open>2.5.1: An IDF-Based Concurrent Separation Logic\<close>

paragraph \<open>The language ParImp\<close>

text \<open>ParImp is defined in the file @{file "FrontEnd/ParImp.thy"}.
\<^item> Syntax:
...
paragraph \<open>The IDF-Based CSL (Figure 2.10)\<close>

text \<open>The program logic (Figure 2.10) is defined in the file @{file "FrontEnd/CSL_IDF.thy"},
as \<^term>\<open>CSL_syn \<Delta> P C Q\<close> or \<^term>\<open>\<Delta> \<turnstile>CSL [P] C [Q]\<close>
\<close>


paragraph \<open>Definition 2.5.1 (Validity of CSL triples)\<close>

text \<open>The following elements are defined as part of definition 2.5.1:
\<^item> \<^term>\<open>accesses C s\<close>
\<^item> \<^term>\<open>writes C s\<close>
\<^item> \<^term>\<open>read_dom \<omega>\<close>
\<^item> \<^term>\<open>write_dom \<omega>\<close>
\<^item> State \<omega> corresponding to a heap h:
  \<^item> The first conjunct corresponds to \<^term>\<open>concretize s \<omega>\<close>, which removes the permission part from \<omega>.
  \<^item> The second conjunct corresponds to \<^term>\<open>binary_mask \<omega>\<close>
\<^item> \<^term>\<open>safe n C s \<omega> Q\<close>
\<^item> \<^term>\<open>CSL \<Delta> P C Q\<close>
\<close>
...
paragraph \<open>Theorems 2.5.1 and 2.5.2: Adequacy and Soundness\<close>

text \<open>The soundness of the CSL (Theorem 2.5.2) is given by the following:\<close>
...
text \<open>Adequacy (combining Theorems 2.5.1 and 2.5.2) is then given by:\<close>
...
subsection \<open>2.5.2: A Sound Front-End Translation\<close>


paragraph \<open>Front-End Translation (Figure 2.11)\<close>

text \<open>The translation from Figure 2.11 is defined in the file @{file "FrontEnd/SyntacticTranslation.thy"}.
For historical reasons, we first do a "semantic" translation (into CoreIVL), in @{file "FrontEnd/FrontEndTranslation.thy"},
...
paragraph \<open>Theorem 2.5.3: Soundness of the front-end translation\<close>
...
text \<open>Note that, as in other results, we have ignored well-formedness and typedness in the thesis 
...
paragraph \<open>Lemma 2.5.4: Inhale-translation-exhale pattern\<close>

text \<open>Lemma 2.5.4: We define convertible as follows.\<close>
...
have a valid derivation in the axiomatic semantics, whereas in the thesis we only require that they
...
lemma inhale_translation_exhale:
  assumes "convertible \<Delta> tys C"
      and "ConcreteSemantics.SL_proof (tcfe \<Delta> tys) P (abs_stmt.Inhale A;; fst (translate \<Delta> tys C);; abs_stmt.Exhale B) Q"
...
paragraph \<open>Lemma 2.5.5: Exhale-havoc-inhale\<close>

lemma exhale_havoc_inhale_pattern:
  assumes context_well_formed: "wrC C \<subseteq> dom (variables \<Delta>) \<and> finite_context \<Delta>"
