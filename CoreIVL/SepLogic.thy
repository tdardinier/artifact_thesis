...
definition upper_closure :: "'a set \<Rightarrow> 'a set" where
...
proof (rule up_closedI)
  ...
  then obtain a where "a \<in> A \<and> \<phi> \<succeq> a"
    using upper_closure_def by fastforce
...
next
  ...
  ultimately show ?A
    by (simp add: pre_pcm_class.pure_def pcm_with_core_class.core_is_pure pcm_with_core_class.prove_in_up_close_core)
qed
...
lemma in_starI:
  assumes "a \<in> A"
      and "b \<in> B"
      and "Some \<omega> = a \<oplus> b"
    shows "\<omega> \<in> A \<otimes> B"
  using assms local.x_elem_set_product by auto
