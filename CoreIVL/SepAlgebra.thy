theory SepAlgebra
  imports SepAlgebraDef PosReal PosRat
begin
...
instance proof
  ...
  show "\<alpha> \<odot> (\<beta> \<odot> x) = \<alpha> * \<beta> \<odot> x"
    by (simp add: mult_mult mult_prod_def)
  show "Some ((\<alpha> + \<beta>) \<odot> x) = \<alpha> \<odot> x \<oplus> \<beta> \<odot> x"
...
instance proof
  ...
  show "1 \<odot> f = f"
  proof (rule ext)
    fix x show "(1 \<odot> f) x = f x"
      by (simp add: mult_fun_def unit_mult)
  qed
  show "\<alpha> \<odot> (\<beta> \<odot> f) = \<alpha> * \<beta> \<odot> f"
  proof (rule ext)
    fix x show "(\<alpha> \<odot> (\<beta> \<odot> f)) x = (\<alpha> * \<beta> \<odot> f) x"
  ...
  then obtain ff where "Some ff = \<alpha> \<odot> f \<oplus> \<beta> \<odot> f"
    by (simp add: plus_fun_def)
  moreover have "ff = \<alpha> + \<beta> \<odot> f"
  proof (rule ext)
    fix x
    show "ff x = (\<alpha> + \<beta> \<odot> f) x"
  ...
  qed
  ultimately show "Some (\<alpha> + \<beta> \<odot> f) = \<alpha> \<odot> f \<oplus> \<beta> \<odot> f"
    by blast
...
definition stable_option :: "'a option \<Rightarrow> bool" where
  "stable_option = pred_option stable"
                       
definition stabilize_option ::"'a option \<Rightarrow> 'a option" where
  "stabilize_option = map_option stabilize"

lemma stable_optionI:
  assumes "\<And>v. x = Some v \<Longrightarrow> stable v"
  shows "stable x"
by (simp add: assms option.pred_set stable_option_def)

instance proof
  fix x a b c :: "'a option"

  show "sep_algebra_class.stable x \<Longrightarrow> stabilize x = x"
    unfolding stabilize_option_def stable_option_def
    by (meson already_stable map_option_idI option.pred_set)

  show "sep_algebra_class.stable (stabilize x)"
    unfolding stabilize_option_def stable_option_def
    by (simp add: option.pred_set option.set_map)

  show "Some x = a \<oplus> b \<Longrightarrow> Some (stabilize x) = stabilize a \<oplus> stabilize b"
    unfolding stabilize_option_def stable_option_def
    by (smt (verit) core_is_smaller option.discI option.map_disc_iff option.map_sel option.sel plus_option.elims stabilize_sum)

  show "Some x = stabilize x \<oplus> |x|"
    unfolding stabilize_option_def stable_option_def
    by (smt (verit) core_is_pure core_option.elims core_option_None decompose_stabilize_pure option.map_disc_iff option.map_sel option.sel plus_option.elims)

(* TODO: Does not hold, should be a separate class *)
...
lemma stable_prodI:
  assumes "stable (fst a)"
      and "stable (snd a)"
    shows "stable a"
  by (simp add: assms(1) assms(2) stable_prod_def)
...
lemma stable_funI:
  assumes "\<And>l. stable (f l)"
  shows "stable f"
  by (simp add: assms stable_fun)
...
instantiation preal :: sep_algebra
begin

definition stable_preal :: "preal \<Rightarrow> bool" where
  "stable_preal _ \<longleftrightarrow> True"

definition stabilize_preal :: "preal \<Rightarrow> preal" where
  "stabilize_preal p = p"

lemma stable_stabilize_preal[simp]:
  fixes p :: preal
  shows "stable p"
    and "stabilize p = p"
  unfolding stable_preal_def stabilize_preal_def by simp_all

instance proof
  fix x a b c :: preal
  show "sep_algebra_class.stable x \<Longrightarrow> stabilize x = x"
    unfolding stable_preal_def stabilize_preal_def
    by blast
  show "sep_algebra_class.stable (stabilize x)"
    unfolding stable_preal_def stabilize_preal_def
    by blast
  ...
  show "Some a = b \<oplus> stabilize |c| \<Longrightarrow> a = b"
    unfolding stable_preal_def stabilize_preal_def
    using core_preal_def preal_plusE by fastforce
qed
