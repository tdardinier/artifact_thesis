...
proof (rule ext)
  fix x show "add_masks (add_masks a b) c x = add_masks a (add_masks b c) x"
    using group_cancel.add1 by fastforce
qed
...
lemma add_masks_equiv_valid_null:
  ...
  by (metis (no_types, opaque_lifting) add_masks.elims comm_monoid_add_class.add_0 pgte_antisym sum_larger valid_null_def)
...
lemma prat_greater_exists:
  assumes "pgte a b"
  shows "\<exists>r. a = b + r"
  by (metis assms pmax_is prat_gte_padd sup_ge2)
  ...
  proof -
    ...
    proof (rule ext)
      ...
      have "\<pi>' hl = padd (\<pi> hl) (?r hl)"
        by (meson \<open>\<forall>hl. pgte (\<pi>' hl) (\<pi> hl)\<close> prat_greater_exists someI_ex)
...
lemma valid_empty[simp]:
  "valid_mask empty_mask"
  by (simp add: pgt_implies_pgte prat_pnone_pgt)
...
proof (rule valid_maskI)
  show "\<And>hl. pgte pwrite (b hl)"
    by (metis add.right_neutral add_0 add_masks.elims assms(1) assms(2) not_pgte_charact pgte_pgt sum_larger valid_mask.simps)
...
proof -
  ...
  proof
    show "\<forall>hl. update_dm dm \<pi> \<pi>' hl \<longrightarrow> h' hl = h hl \<Longrightarrow> equal_on_mask m h h' \<and> equal_on_bmask dm h h'"
      apply (simp add: assms equal_on_bmask_def equal_on_mask_def pgt.rep_eq ppos.rep_eq update_dm_def)
      by (simp add: plus_prat.rep_eq)
    assume "equal_on_mask m h h' \<and> equal_on_bmask dm h h'"
    then have "\<And>hl. update_dm dm \<pi> \<pi>' hl \<Longrightarrow> h' hl = h hl"
      unfolding equal_on_bmask_def equal_on_mask_def update_dm_def
      using assms pgt.rep_eq plus_prat.rep_eq ppos.rep_eq by auto
...
proof (rule ccontr)
  ...
  then have "pgt (padd (a hl) (b hl)) (padd (c hl) (d hl))"
    by (metis add.commute assms(2) greater_mask_equiv_def pgte_pgt)
  then show "False"
...
proof
  ...
  proof (rule equal_on_maskI)
    ...
    next
      case False
      then show ?thesis
        by (metis \<open>equal_on_mask \<pi>1 h h' \<and> equal_on_mask \<pi>2 h h'\<close> \<open>ppos (add_masks \<pi>1 \<pi>2 hl)\<close> add_masks.elims equal_on_mask_def less_add_same_cancel1 not_pgte_charact pgt.rep_eq pgte_antisym plus_prat.rep_eq ppos.rep_eq sum_larger)
    qed
  qed
qed
...
lemma valid_larger_mask:
  ...
  apply rule
   apply (metis full_mask_def greater_mask_equiv_def not_pgte_charact prat_pgt_pnone prod.exhaust_sel valid_mask.simps)
  by (metis fst_conv full_mask_def greater_mask_equiv_def not_pgte_charact pgt_implies_pgte prat_pnone_pgt valid_mask.simps)
...
lemma mult_greater:
  ...
  using assms greater_mask_equiv_def multiply_mask_def prat_greater_exists pmult_distr
  by (metis (full_types) sum_larger)
...
lemma upper_bounded_smaller:
  ...
  by (metis assms multiply_mask_def prat_greater_exists pmult_distr upper_bounded_def sum_larger)
...
proof (rule valid_maskI)
  ...
  then have "upper_bounded (multiply_mask p \<pi>) (pmult p pwrite)"
    using upper_bounded_smaller by blast
  ...
  show "\<And>f. multiply_mask p \<pi> (null, f) = pnone"
    by (metis add_0 assms(1) multiply_mask_def padd_cancellative pmult_distr valid_mask.simps)
qed
...
proof (rule valid_maskI)
  ...
  proof -
    ...
    have "pgte (padd ma mb) (add_masks a b hl)"
      by (metis ab_semigroup_add_class.add_ac(1) add_masks.elims assms(3) assms(4) not_pgte_charact pgt_implies_pgte pgte_antisym pgte_pgt prat_greater_exists sum_larger upper_bounded_def)
  ...
  show "upper_bounded (add_masks a b) (padd ma mb)"
    using add_mono_thms_linordered_semiring(1) assms(3) assms(4) pgte.rep_eq plus_prat.rep_eq upper_bounded_def by fastforce
qed
...
lemma valid_multiply:
  ...
  by (metis (full_types) add_masks_cancellative assms(1) assms(2) assms(3) minus_empty mult.commute mult_add_states mult_distr_masks multiply_mask_def upper_bounded_bigger upper_bounded_def upper_bounded_smaller valid_empty valid_mask.simps)

lemma greater_mult:
  assumes "greater_mask a b"
  shows "greater_mask (multiply_mask p a) (multiply_mask p b)"
  using mult_greater assms 
  by simp
