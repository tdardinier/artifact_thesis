...
definition mono_transformer :: "'a transformer \<Rightarrow> bool" where
  "mono_transformer T \<longleftrightarrow> (\<forall>\<phi> \<phi>'. \<phi>' \<succeq> \<phi> \<longrightarrow> T \<phi>' \<succeq> T \<phi>) \<and> (\<forall>x. T (stabilize |x| ) = (stabilize |x| ))"
...
proof -
  ...
  then show ?thesis
    by (metis \<open>Some \<phi>' = \<phi> \<oplus> r\<close> assms(1) local.asso1 local.greater_def)
qed
...
proof (induct arbitrary: \<phi> \<phi>' r rule: wf_assertion.induct)
  ...
  ultimately show ?case
    by (metis "1.hyps"(2) "1.prems"(1) "1.prems"(3) "1.prems"(4) \<open>Some \<phi> = a \<oplus> b\<close> \<open>sat A a\<close> \<open>sat B b\<close> asso1 sat.simps(1) wf_assertion.simps(1))
next
  case (2 b A)
  then show ?case
...
lemma defined_sum_move:
  assumes "a ## b"
    ...
    shows "a' ## y"
  by (metis assms(1,2,3) local.asso1 local.defined_def)

lemma bigger_core_sum_defined:
  assumes "|a| \<succeq> b"
  shows "Some a = a \<oplus> b"
  by (metis assms asso1 commutative core_is_pure max_projection_prop_pure_core minus_exists mpp_smaller option.distinct(1) option.exhaust option.inject positivity succ_trans)
...
proof (induct rule: package_rhs.induct)
  ...
  proof (rule package_rhs_connectionI)
    ...
    then obtain au where asm2: "Some au = a \<oplus> u \<and> (au ## (T f' \<ominus> T f) \<longrightarrow>
      (\<exists>a' u'. (a', u', T) \<in> S' \<and> |a'| \<succeq> |a| \<and> au \<oplus> (T f' \<ominus> T f) = a' \<oplus> u' \<and> u' \<succeq> u \<and> package_sat ?pc A a' u' u))"
      using \<open>\<And>u a T. (a, u, T) \<in> S \<Longrightarrow> \<exists>au. Some au = a \<oplus> u \<and> (au ## (T f' \<ominus> T f) \<longrightarrow> (\<exists>a' u'. (a', u', T) \<in> S' \<and> |a'| \<succeq> |a| \<and> au \<oplus> (T f' \<ominus> T f) = a' \<oplus> u' \<and> u' \<succeq> u \<and> package_sat (bool_conj pc b) A a' u' u))\<close> by presburger
    ...
    proof -
      ...
      proof -
        ...
        then have "a' ## (u' \<ominus> u)"
          by (metis \<open>(a', u', T) \<in> S' \<and> |a'| \<succeq> |a| \<and> au \<oplus> (T f' \<ominus> T f) = a' \<oplus> u' \<and> u' \<succeq> u \<and> package_sat (bool_conj pc b) A a' u' u\<close> asm3 asso3 defined_def minus_exists)
        then show ?thesis
          by (metis local.asso2 local.commutative local.core_is_smaller local.defined_def local.greater_equiv option.collapse)
      qed
      ...
      proof (rule package_satI)
        ...
        proof (cases "b |a'|")
          ...
          then show ?thesis
            by (metis \<open>(a', u', T) \<in> S' \<and> |a'| \<succeq> |a| \<and> au \<oplus> (T f' \<ominus> T f) = a' \<oplus> u' \<and> u' \<succeq> u \<and> package_sat (bool_conj pc b) A a' u' u\<close> package_sat_def sat.simps(2))
        next
          case False
          ...
          moreover obtain x where "Some x = |a'| \<oplus> (u' \<ominus> u)"
            by (metis \<open>(a', u', T) \<in> S' \<and> |a'| \<succeq> |a| \<and> au \<oplus> (T f' \<ominus> T f) = a' \<oplus> u' \<and> u' \<succeq> u \<and> package_sat (bool_conj pc b) A a' u' u\<close> asm3 asso2 commutative core_is_smaller defined_def minus_and_plus option.collapse)
      ...
      then show "\<exists>a' u'. (a', u', T) \<in> S' \<and> |a'| \<succeq> |a| \<and> au \<oplus> (T f' \<ominus> T f) = a' \<oplus> u' \<and> u' \<succeq> u \<and> package_sat pc (AImp b A) a' u' u"
        using \<open>(a', u', T) \<in> S' \<and> |a'| \<succeq> |a| \<and> au \<oplus> (T f' \<ominus> T f) = a' \<oplus> u' \<and> u' \<succeq> u \<and> package_sat (bool_conj pc b) A a' u' u\<close> by blast
    qed
    ...
    proof (cases "au ## (T f'' \<ominus> T f)")
      ...
      proof (rule package_satI)
        ...
        moreover have "Some |a''| = |a'| \<oplus> |a''|"
          by (metis (no_types, lifting) calculation(3) core_is_pure local.asso1 local.minus_exists)
    ...
    next
      ...
      proof -
        have "a \<succeq> |T f|" using \<open>(a, u, T) \<in> S\<close> \<open>valid_package_set S f\<close> valid_package_set_def
          by fastforce
        then show ?thesis
          by (metis \<open>a' = a \<ominus> b\<close> local.max_projection_prop_pure_core local.minus_core local.mpp_invo local.mpp_mono local.mpp_smaller
             local.succ_trans)
      qed
  ...
  proof (rule package_rhs_connectionI)
      ...
      proof (cases "pc a")
        ...
        proof (rule package_satI)
          ...
          have "Some ?u = u \<oplus> ?b"
            by (metis (no_types, lifting) \<open>Some au = a \<oplus> u\<close> calculation(1) commutative minus_equiv_def option.distinct(1) option.exhaust_sel asso3)
          moreover have "?a ## ?u"
    ...
    proof -
      ...
      then have "|a| \<succeq> T f \<ominus> T f"
        using local.core_is_smaller local.max_projection_prop_def local.max_projection_prop_pure_core local.minusI by presburger
...
next
  ...
  proof (rule valid_package_setI)
    ...
    then have "|a| \<succeq> |u| \<and> mono_transformer T \<and> a \<succeq> |T f|" using \<open>valid_package_set S f\<close> valid_package_set_def 
      by fastforce
    moreover have "a' \<succeq> |T f'|"
      by (metis \<open>\<And>thesis. (\<And>a. (a, u, T) \<in> S \<Longrightarrow> a' ## u \<Longrightarrow> Some a' = a \<oplus> (T f' \<ominus> T f) \<Longrightarrow> thesis) \<Longrightarrow> thesis\<close> local.greater_equiv
          local.max_projection_prop_pure_core local.minus_core local.mpp_smaller local.succ_trans)
...
proof -
  ...
  then have "Some a = r' \<oplus> b2"
    by (metis \<open>Some a = r \<oplus> b2 \<and> r \<succeq> b1\<close> commutative core_is_smaller asso1)
  then show ?thesis
...
proof -
  ...
  moreover have "|a|  ## x"
    by (metis assms(2) calculation defined_def asso3 minus_exists)
  ultimately show ?thesis
...
lemma wf_assertion_and_the:
  ...
  by (metis assms(1) assms(2) assms(3) commutative defined_def max_projection_prop_pure_core option.collapse mpp_prop wf_assertion_sat_larger_pure)
...
next
  ...
  proof (rule package_rhs.ASem)
    ...
    proof -
      ...
      then show ?thesis
        by (metis calculation(2) commutative max_projection_prop_pure_core mpp_smaller mpp_prop smaller_pure_sum_smaller)
    qed
    ...
    next
      ...
      proof -
        ...
        proof -
          ...
          ultimately show ?thesis
            by (metis commutative max_projection_prop_pure_core mpp_smaller mpp_prop smaller_pure_sum_smaller)
        qed
        then show ?thesis
...
next
  ...
  proof -
    ...
    moreover have "|a| ## x \<and> |a| ## y"
      by (metis \<open>Some af = |a| \<oplus> f1 a u T\<close> calculation commutative defined_def option.discI asso3)
    ...
    ultimately have "pc |a| \<Longrightarrow> sat A (the ( |a| \<oplus> x)) \<and> sat B (the ( |a| \<oplus> y))"
      by (metis AStar.prems(3) \<open>pc |a| \<longrightarrow> sat A x\<close> \<open>pc |a| \<longrightarrow> sat B y\<close> \<open>|a| ## x \<and> |a| ## y\<close> commutative defined_def max_projection_prop_pure_core option.exhaust_sel wf_assertion.simps(1) mpp_prop wf_assertion_sat_larger_pure)
...
\<open>Some (f1 a u T) = ?fA a u T \<oplus> yy \<and> |?fA a u T| \<succeq> |f1 a u T| \<and> |yy| \<succeq> |a| \<and> (pc |a| \<longrightarrow> sat A (the ( |a| \<oplus> ?fA a u T)) \<and> sat B (the ( |a| \<oplus> yy)))\<close>
  ...
  proof (rule AStar(2))
    ...
    proof -
      ...
      then show ?thesis
        using max_projection_propE(3) max_projection_prop_pure_core mpp_prop by fastforce
    qed
...
lemma property_needed_one:
  "stabilize  |a| \<oplus> a = Some a"
  by (simp add: local.commutative local.stabilize_core_right_id)

lemma compatible_with_stab_pure_larger:
  assumes "a ## stabilize |b|"
  shows "a \<succeq> stabilize |b|"
  by (metis assms local.defined_def local.greater_equiv local.splus.elims local.stabilize_core_emp)

lemma stab_pure_is_pure:
  "|stabilize |\<phi>|| = stabilize |\<phi>|"
  by (metis local.commutative local.decompose_stabilize_pure local.pure_larger_stabilize local.pure_larger_stabilize_same local.stabilize_core_emp)

subsection Theorems

theorem general_soundness:
  assumes "package_rhs \<phi> (stabilize |\<phi>| ) { (a, stabilize |a|, T) |a T. (a, T) \<in> S \<and> a ## stabilize |\<phi>| } (\<lambda>_. True) A \<phi>' f S'"
...
proof -
  let ?S = "{ (a, stabilize |a|, p) |a p. (a, p) \<in> S \<and> a ## stabilize |\<phi>| }"
  let ?pc = "\<lambda>_. True"
  have prc: "package_rhs_connection \<phi> (stabilize |\<phi>| ) ?S ?pc A \<phi>' f S' \<and> valid_package_set S' f"
  proof (rule package_rhs_proof)
    show "package_rhs \<phi> (stabilize |\<phi>| ) ?S (\<lambda>_. True) A \<phi>' f S'"
      using assms(1) by auto
    show "valid_package_set ?S (stabilize  |\<phi>| )"
    proof (rule valid_package_setI)
      fix a u T
      assume "(a, u, T) \<in> {(a, stabilize  |a| , p) |a p. (a, p) \<in> S \<and> a ## stabilize |\<phi>| }"
      then have "u = stabilize  |a|" by blast
      show "a ## u \<and>  |a|  \<succeq>  |u|  \<and> mono_transformer T \<and> a \<succeq>  |T (stabilize  |\<phi>| )| "
        apply (intro conjI)
        using \<open>u = stabilize |a|\<close> commutative defined_def property_needed_one apply auto[1]
          apply (metis \<open>u = stabilize |a|\<close> commutative core_sum greater_equiv property_needed_one)
        using \<open>(a, u, T) \<in> {(a, stabilize |a|, p) |a p. (a, p) \<in> S \<and> a ## stabilize |\<phi>|}\<close> assms(2) apply blast
        by (smt (verit, best) CollectD Pair_inject \<open>(a, u, T) \<in> {(a, stabilize |a|, p) |a p. (a, p) \<in> S \<and> a ## stabilize |\<phi>|}\<close> assms(2) bigger_core_sum_defined compatible_with_stab_pure_larger core_mono local.greater_equiv mono_transformer_def)
    qed
    show "wf_assertion A" by (simp add: assms(3))
    show "mono_pure_cond (\<lambda>_. True)"
      using mono_pure_cond_def by auto
    show "stable (stabilize  |\<phi>| )" by simp
    show "\<phi> ## stabilize  |\<phi>|"
      using commutative defined_def property_needed_one by auto
  qed

  then obtain r: "\<phi> \<oplus> stabilize  |\<phi>| = \<phi>' \<oplus> f" "stable f"
    "\<And>a u T. (a, u, T) \<in> ?S \<Longrightarrow> (\<exists>au. Some au = a \<oplus> u \<and> (au ## (T f \<ominus> T (stabilize  |\<phi>| )) \<longrightarrow>
      (\<exists>a' u'. (a', u', T) \<in> S' \<and> |a'| \<succeq> |a| \<and> au \<oplus> (T f \<ominus> T ( stabilize  |\<phi>| )) = a' \<oplus> u' \<and> u' \<succeq> u \<and> package_sat ?pc A a' u' u)))"
    using package_rhs_connection_def by force

  moreover have "\<And>a T x. (a, T) \<in> S \<and> Some x = a \<oplus> T f \<Longrightarrow> a ## stabilize |\<phi>| \<Longrightarrow> sat A x"
  proof -
    fix a T x assume asm0: "(a, T) \<in> S \<and> Some x = a \<oplus> T f" "a ## stabilize |\<phi>|"
    then have "T f \<ominus> T (stabilize  |\<phi>| ) = T f"
      by (metis assms(2) local.commutative local.minus_default local.stabilize_core_emp minus_some mono_transformer_def)

    then obtain au where "Some au = a \<oplus> stabilize  |a| \<and> (au ## T f \<longrightarrow>
      (\<exists>a' u'. (a', u', T) \<in> S' \<and> |a'| \<succeq> |a| \<and> au \<oplus> T f = a' \<oplus> u' \<and> u' \<succeq> stabilize  |a| \<and> package_sat ?pc A a' u' (stabilize  |a| )))"
      using r asm0 by fastforce
    then have "au = a" 
      using commutative property_needed_one by force

    then have "(\<exists>a' u'. (a', u', T) \<in> S' \<and> |a'| \<succeq> |a| \<and> a \<oplus> T f = a' \<oplus> u' \<and> package_sat ?pc A a' u' (stabilize  |a| ))"
      using \<open>Some au = a \<oplus> stabilize |a| \<and> (au ## T f \<longrightarrow> (\<exists>a' u'. (a', u', T) \<in> S' \<and> |a'| \<succeq> |a| \<and> au \<oplus> T f = a' \<oplus> u' \<and> u' \<succeq> stabilize |a| \<and> package_sat (\<lambda>_. True) A a' u' (stabilize |a| )))\<close> asm0 defined_def by auto
    then obtain a' u' where a'u'_def: "(a', u', T) \<in> S' \<and> |a'| \<succeq> |a| \<and> a \<oplus> T f = a' \<oplus> u' \<and> package_sat ?pc A a' u' (stabilize  |a| )"
      by presburger
    then obtain y where "Some y = |a'| \<oplus> (u' \<ominus> (stabilize  |a| )) \<and> sat A y"
      using package_sat_def by auto
    then have "Some y =  |a'| \<oplus> u'"
      by (metis local.commutative local.minus_default local.stabilize_core_emp minus_some)
    then have "x \<succeq> y"
      using \<open>(a', u', T) \<in> S' \<and> |a'| \<succeq> |a| \<and> a \<oplus> T f = a' \<oplus> u' \<and> package_sat (\<lambda>_. True) A a' u' (stabilize |a| )\<close> asm0
      by (metis local.addition_bigger local.max_projection_prop_pure_core local.mpp_smaller)
    then show "sat A x"
    proof (cases "intuitionistic (sat A)")
      case True
      then show ?thesis
        by (meson \<open>Some y = |a'| \<oplus> (u' \<ominus> stabilize |a| ) \<and> sat A y\<close> \<open>x \<succeq> y\<close> intuitionistic_def)
    next
      case False
      then have "pure_remains S'" using assms(4) by auto
      then have "pure a'"
        using pure_remains_def property_needed_one pure_def a'u'_def
        by fastforce
      then show ?thesis
        using \<open>Some y = |a'| \<oplus> (u' \<ominus> stabilize |a| ) \<and> sat A y\<close> minus_some option.sel property_needed_one
        by (metis \<open>Some y = |a'| \<oplus> u'\<close> a'u'_def asm0(1) local.max_projection_prop_def local.max_projection_prop_pure_core local.mppI local.succ_refl)
    qed
  qed

  moreover have "\<And>a T. (a, T) \<in> S \<Longrightarrow> a ## T f \<Longrightarrow> a ## stabilize |\<phi>|"
  proof -
    fix a T assume asm0: "(a, T) \<in> S" "a ## T f"
    moreover have "T f \<succeq> stabilize |\<phi>|"
    proof -
      have "f \<succeq> stabilize |\<phi>|"
        using package_rhs_connection_def prc by blast
      then show "T f \<succeq> stabilize |\<phi>|"
        by (metis assms(2) calculation(1) mono_transformer_def)
    qed
    then show "a ## stabilize |\<phi>|"
      using calculation(2) local.defined_comm local.smaller_compatible by blast
  qed
  ultimately have "(\<forall>(a, T) \<in> S. a ## T f \<longrightarrow> sat A (the (a \<oplus> T f)))"
    using defined_def 
    by force
  moreover have "Some \<phi> = \<phi>' \<oplus> f \<and> stable f"
    using commutative property_needed_one r(1,2) by presburger
  ultimately show "Some \<phi> = \<phi>' \<oplus> f \<and> sep_algebra_class.stable f \<and> (\<forall>(a, T)\<in>S. a ## T f \<longrightarrow> sat A (the (a \<oplus> T f)))" by blast
qed
...
theorem completeness:
  assumes "\<And>a u T x. (a, u, T) \<in> S \<Longrightarrow> Some x = a \<oplus> T f \<Longrightarrow> sat A x"
      ...
      and "stable f"
      and "\<And>a u T. (a, u, T) \<in> S \<Longrightarrow> mono_transformer T"
      and "wf_assertion A"
    shows "\<exists>S'. package_rhs \<phi> (stabilize |\<phi>| ) S (\<lambda>_. True) A \<phi>' f S'"
proof -
  define S' where "S' = { (r, u, T) |a u T r. (a, u, T) \<in> S \<and> Some r = a \<oplus> (T f \<ominus> T (stabilize |\<phi>| )) \<and> r ## u }"
  let ?pc = "\<lambda>_. True"
  have "\<exists>S''. package_rhs \<phi>' f S' ?pc A \<phi>' f S''"
  proof -
    let ?f2 = "\<lambda>a u T. (stabilize |\<phi>| )"
    ...
    proof (rule completeness_aux)
      ...
      fix a u T
      assume "(a, u, T) \<in> S'"
      then obtain a' u' where "(a', u', T) \<in> S" "Some a = a' \<oplus> (T f \<ominus> T (stabilize |\<phi>| ))" using S'_def by blast
      moreover have "T f \<ominus> T (stabilize |\<phi>| ) = T f"
      proof -
        have "mono_transformer T"
          using assms(4) calculation(1) by blast
        then show ?thesis
          by (metis local.commutative local.minus_default local.stabilize_core_emp minus_some mono_transformer_def)
      qed

      then have "sat A (the ( |a|  \<oplus> a))"
        by (metis assms(1) calculation(1,2) local.commutative local.core_is_smaller option.sel)
      moreover have "Some a = a \<oplus> (stabilize |\<phi>| )"
      proof -
        have "a \<succeq> T f \<ominus> T (stabilize |\<phi>| )"
          using calculation(2) local.greater_equiv by blast
        moreover have "f \<succeq> stabilize |\<phi>|"
        proof -
          obtain ff where "Some ff = f \<oplus> stabilize |\<phi>|"
            using assms(2) local.compatible_smaller local.greater_equiv local.stabilize_core_right_id by blast
          then show ?thesis
            using local.greater_equiv local.stabilize_core_emp by blast
        qed
        moreover have "mono_transformer T"
          using \<open>(a', u', T) \<in> S\<close> assms(4) by force

        moreover have "T f \<succeq> stabilize |\<phi>|"
          by (metis calculation(2,3) mono_transformer_def)
        moreover have "T f \<ominus> T (stabilize |\<phi>| ) \<succeq> stabilize |\<phi>|"
          by (smt (verit) \<open>T f \<ominus> T (stabilize |\<phi>| ) = T f\<close> calculation(4)) 
        then obtain aa where "Some a = aa \<oplus> (stabilize |\<phi>| )"
          using calculation(1) local.greater_equiv local.succ_trans by blast
        then have "a = aa"
          by (simp add: local.stabilize_core_emp)
        ultimately show ?thesis
          using \<open>Some a = aa \<oplus> stabilize |\<phi>|\<close> by fastforce
      qed
      ultimately show "|a|  \<succeq>  |a|  \<and> Some a = a \<oplus> (stabilize |\<phi>| ) \<and> (True \<longrightarrow> sat A (the ( |a|  \<oplus> a)))"
        using local.succ_refl by blast
    qed (simp)
    then show ?thesis by auto
  qed
  then obtain S'' where "package_rhs \<phi>' f S' ?pc A \<phi>' f S''" by blast
  have "package_rhs \<phi> (stabilize |\<phi>| ) S ?pc A \<phi>' f S''"
    using assms(2)
  proof (rule package_rhs.AddFromOutside)
    ...
    show "Some f = (stabilize |\<phi>| ) \<oplus> f"
      by (metis assms(2) local.commutative local.core_sum local.plus_pure_stabilize_eq local.stabilize_core_right_id)
   show "S' = { (r, u, T) |a u T r. (a, u, T) \<in> S \<and> Some r = a \<oplus> (T f \<ominus> T (stabilize |\<phi>| )) \<and> r ## u }"
      using S'_def by blast
  qed
  then show ?thesis
    by blast
