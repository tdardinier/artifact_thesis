...
lemma convert_proof_skip:
  assumes "ConcreteSemantics.SL_proof (tcfe \<Delta> tys) A AbstractSemantics.abs_stmt.Skip B"
...
lemma convert_proof_alloc:
  assumes "ConcreteSemantics.SL_proof (tcfe \<Delta> tys) P (AbstractSemantics.Havoc r;; abs_stmt.Inhale (Stabilize (full_ownership_with_val r e))) Q"
...
proof (rule RuleConsTyped)
  ...
  have asm0: "ConcreteSemantics.SL_proof (tcfe \<Delta> tys) P (AbstractSemantics.Havoc r) (TypedEqui.exists_assert (tcfe \<Delta> tys) r P)
  \<and> ConcreteSemantics.SL_proof (tcfe \<Delta> tys) (TypedEqui.exists_assert (tcfe \<Delta> tys) r P) (abs_stmt.Inhale (Stabilize (full_ownership_with_val r e))) Q
  \<and> self_framing P"
...
lemma helper_exhale_encoding:
  assumes "ConcreteSemantics.SL_proof (tcfe \<Delta> tys) P (abs_stmt.Exhale (Stabilize A)) Q"
...
lemma convert_proof_free:
  assumes "ConcreteSemantics.SL_proof (tcfe \<Delta> tys) P (abs_stmt.Exhale (Stabilize (full_ownership r))) Q"
    ...
    proof (rule RuleConsTyped)
      ...
      proof (rule ConcreteSemantics.entails_typedI)
        ...
        moreover obtain v' where "get_h \<omega> (l, field_val) = Some v'"
          by (metis gr_0_is_ppos \<open>get_m \<omega> (l, field_val) = 1 \<and> semantify_addr r \<omega> = Some l\<close> not_gr_0 vstate_wf_Some zero_neq_one)
...
lemma convert_proof_read:
  assumes "ConcreteSemantics.SL_proof (tcfe \<Delta> tys) P (abs_stmt.LocalAssign x (semantify_heap_loc r)) Q"
...
proof (rule ConcreteSemantics.SL_proof_LocalAssign_elim)
  ...
  proof -
    ...
    then have "get_m \<omega> (l, field_val) > 0"
      using gr_0_is_ppos stable_virtual_state_def by fastforce
...
fun translate :: "('a, 'a virtual_state) interp \<Rightarrow> vtyp list \<Rightarrow> cmd \<Rightarrow> 'a v_stmt \<times> 'a v_stmt set"
  where
  "translate \<Delta> tys Cskip = (abs_stmt.Skip, {})"
| "translate \<Delta> tys (Cassign x e) = (abs_stmt.LocalAssign x (semantify_exp e), {})"

| "translate \<Delta> tys (Calloc r e) = ((abs_stmt.Havoc r;; abs_stmt.Inhale (Stabilize (full_ownership_with_val r e))), {})"
| "translate \<Delta> tys (Cfree r) = (abs_stmt.Exhale (Stabilize (full_ownership r)), {})"
| "translate \<Delta> tys (Cwrite r e) = (Custom (FieldAssign (semantify_addr r) field_val (semantify_exp e)), {})"
| "translate \<Delta> tys (Cread x r) = (abs_stmt.LocalAssign x (semantify_heap_loc r), {})"
...
| "translate \<Delta> tys (Cif b C1 C2) = (abs_stmt.If (semantify_bexp b) (fst (translate \<Delta> tys C1)) (fst (translate \<Delta> tys C2)), snd (translate \<Delta> tys C1) \<union> snd (translate \<Delta> tys C2))"

| "translate \<Delta> tys ({P1} C1 {Q1} || {P2} C2 {Q2}) =
  ((abs_stmt.Exhale (inhalify \<Delta> tys (make_semantic_assertion \<Delta> (tcfes tys) P1 \<otimes> make_semantic_assertion \<Delta> (tcfes tys) P2));;
  ConcreteSemantics.havoc_list (wrL C1 @ wrL C2);;
  abs_stmt.Inhale (make_semantic_assertion \<Delta> (tcfes tys) Q1 \<otimes> make_semantic_assertion \<Delta> (tcfes tys) Q2)),
  let r1 = translate \<Delta> tys C1 in let r2 = translate \<Delta> tys C2 in
  { (abs_stmt.Inhale (make_semantic_assertion \<Delta> (tcfes tys) P1);; fst r1;; abs_stmt.Exhale (inhalify \<Delta> tys (make_semantic_assertion \<Delta> (tcfes tys) Q1))),
   (abs_stmt.Inhale (make_semantic_assertion \<Delta> (tcfes tys) P2);; fst r2;; abs_stmt.Exhale (inhalify \<Delta> tys (make_semantic_assertion \<Delta> (tcfes tys) Q2))) } \<union> snd r1 \<union> snd r2)"

| "translate \<Delta> tys (Cwhile b I C) = (abs_stmt.Exhale (inhalify \<Delta> tys (make_semantic_assertion \<Delta> (tcfes tys) I));;
  ConcreteSemantics.havoc_list (wrL C);; abs_stmt.Inhale ((make_semantic_assertion \<Delta> (tcfes tys) I) \<inter> assertify_bexp (Bnot b)),
  { abs_stmt.Inhale ((make_semantic_assertion \<Delta> (tcfes tys) I) \<inter> assertify_bexp b);; fst (translate \<Delta> tys C);; abs_stmt.Exhale (inhalify \<Delta> tys (make_semantic_assertion \<Delta> (tcfes tys) I)) } \<union> snd (translate \<Delta> tys C))"
...
proof (rule invariant_translateI)
  ...
  moreover obtain B1 where B1_def: "ConcreteSemantics.SL_proof (tcfe \<Delta> tys) (atrue \<Delta> tys)
    (abs_stmt.Inhale ?P1;; fst (translate \<Delta> tys C1);; abs_stmt.Exhale (inhalify \<Delta> tys ?Q1)) B1"
  ...
  (abs_stmt.Inhale ?P2;; fst (translate \<Delta> tys C2);; abs_stmt.Exhale (inhalify \<Delta> tys ?Q2)) B2"
    using asm0 unfolding translate.simps proof_obligations_valid_def
    by (metis Un_iff insertCI snd_eqD)
  moreover have main: "ConcreteSemantics.SL_proof (tcfe \<Delta> tys) P (abs_stmt.Exhale (inhalify \<Delta> tys (?P1 \<otimes> ?P2));;
  ConcreteSemantics.havoc_list (wrL C1 @ wrL C2);; abs_stmt.Inhale (?Q1 \<otimes> ?Q2)) Q"
    using asm0 by simp
...
proof (rule invariant_translateI)
  ...
  moreover obtain B where B_def: "ConcreteSemantics.SL_proof (tcfe \<Delta> tys) (atrue \<Delta> tys) (abs_stmt.Inhale (?I \<inter> assertify_bexp b);; fst (translate \<Delta> tys C);; abs_stmt.Exhale (inhalify \<Delta> tys ?I)) B"    
...
next
  ...
  proof (rule invariant_translate_parallel)
    ...
    proof (rule Cpar(1))
      have "ConcreteSemantics.wf_abs_stmt (tcfe \<Delta> tys) (abs_stmt.Inhale ?P1;; fst (translate \<Delta> tys C1);; abs_stmt.Exhale (inhalify \<Delta> tys ?Q1))"
    ...
    proof (rule Cpar(2))
      have "ConcreteSemantics.wf_abs_stmt (tcfe \<Delta> tys) (abs_stmt.Inhale ?P2;; fst (translate \<Delta> tys C2);; abs_stmt.Exhale (inhalify \<Delta> tys ?Q2))"
        by (metis Cpar.prems(4) Un_iff insertCI snd_eqD translate.simps(9))
...
next
  ...
  proof (rule invariant_translate_while)
    ...
    proof (rule Cwhile(1)[of P Q])
      ...
      have "ConcreteSemantics.wf_abs_stmt (tcfe \<Delta> tys) (abs_stmt.Inhale (?I \<inter> assertify_bexp b);; fst (translate \<Delta> tys C);; abs_stmt.Exhale (inhalify \<Delta> tys ?I))"
...
fun no_old_stmt where
  "no_old_stmt abs_stmt.Skip \<longleftrightarrow> True"
| "no_old_stmt (C1;; C2) \<longleftrightarrow> no_old_stmt C1 \<and> no_old_stmt C2"
| "no_old_stmt (abs_stmt.If b C1 C2) \<longleftrightarrow> indep_trace_exp b \<and> no_old_stmt C1 \<and> no_old_stmt C2"
...
| "no_old_stmt (abs_stmt.Inhale A) \<longleftrightarrow> indep_trace_assertion A"
| "no_old_stmt (abs_stmt.Exhale A) \<longleftrightarrow> indep_trace_assertion A"
| "no_old_stmt (abs_stmt.LocalAssign x e) \<longleftrightarrow> indep_trace_exp e"
| "no_old_stmt (abs_stmt.Havoc x) \<longleftrightarrow> True"
...
proof (induct rule: ConcreteSemantics.red_stmt_sequential_composition.inducts)
  ...
  then show ?case 
next
  case (Redabs_stmt.Skip \<Delta> \<omega>)
  then show ?case 
    using ConcreteSemantics.Redabs_stmt.Skip by blast
next
  case (Redabs_stmt.Inhale \<omega> A \<Delta>)
  then show ?case 
next
  case (Redabs_stmt.Exhale a A \<omega> \<omega>' \<Delta>)
  then show ?case 
next
...
next
  ...
  then show ?case 
next
  case (Redabs_stmt.LocalAssign \<Delta> x ty e \<omega> v)
  then show ?case 
next
  case (Redabs_stmt.Havoc \<Delta> x ty \<omega>)
  then show ?case 
next
...
theorem sound_translation:
  assumes "wf_stmt \<Delta> tys C"
      ...
      and ver_main: "ConcreteSemantics.verifies_set (tcfe \<Delta> tys) (atrue \<Delta> tys) (abs_stmt.Inhale P;; fst (translate \<Delta> tys C);; abs_stmt.Exhale Q)"
...
proof -
  ...
  obtain B where "ConcreteSemantics.SL_proof (tcfe \<Delta> tys) (atrue \<Delta> tys) (abs_stmt.Inhale P;; fst (translate \<Delta> tys C);; abs_stmt.Exhale Q) B"
