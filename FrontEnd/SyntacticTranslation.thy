theory SyntacticTranslation
  imports FrontEndTranslation "../CoreIVL/AbstractSemanticsProperties"
begin                  

section \<open>Refinement Calculus\<close>

(* C1 verifies more than C2 *)
(* abs_stmt.If C2 verifies, then C1 verifies as well *)
(* C1 is more refined *)
...
lemma verifies_more_seq:
  assumes "verifies_more \<Gamma> C1 C1'"
    ...
    shows "verifies_more \<Gamma> (abs_stmt.Seq C1 C2) (abs_stmt.Seq C1' C2')"
...
lemma verifies_more_if:
  assumes "verifies_more \<Gamma> C1 C1'"
    ...
    shows "verifies_more \<Gamma> (abs_stmt.If b C1 C2) (abs_stmt.If b' C1' C2')"
proof (rule verifies_moreI)
  fix \<omega> S'
  assume asm0: "sep_algebra_class.stable \<omega>" "typed \<Gamma> \<omega>"
...
lemma verifies_more_local_assign:
  assumes "exp_refined_by \<Gamma> e e'"
  shows "verifies_more \<Gamma> (abs_stmt.LocalAssign x e) (abs_stmt.LocalAssign x e')"
...
(* assumes "\<And>\<omega>' \<omega> a. sep_algebra_class.stable \<omega> \<Longrightarrow> typed \<Gamma> \<omega> \<Longrightarrow> a \<in> A' \<Longrightarrow> Some \<omega> = \<omega>' \<oplus> a \<Longrightarrow> sep_algebra_class.stable \<omega>' \<Longrightarrow> a \<in> A" *)
  shows "verifies_more \<Gamma> (abs_stmt.Exhale A) (abs_stmt.Exhale A')"
proof (rule verifies_moreI)
  fix \<omega> S'
  assume asm0: "sep_algebra_class.stable \<omega>" "typed \<Gamma> \<omega>"
...
sep_algebra_class.stable \<omega>' \<Longrightarrow> typed \<Gamma> \<omega>' \<Longrightarrow> (\<exists>a' \<in> A'. Some \<omega>' = \<omega> \<oplus> a')"
  shows "verifies_more \<Gamma> (abs_stmt.Inhale A) (abs_stmt.Inhale A')"
proof (rule verifies_moreI)
  fix \<omega> S'
  assume asm0: "sep_algebra_class.stable \<omega>" "typed \<Gamma> \<omega>"
...
next
  ...
  then show ?case
    ...
    apply (subgoal_tac "verifies_more (tcfe \<Delta> tys) (fst (translate \<Delta> tys C1) ;; fst (translate \<Delta> tys C2))
     (abs_stmt.Seq (compile \<Delta> (tcfes tys) (fst (translate_syn C1))) (compile \<Delta> (tcfes tys) (fst (translate_syn C2))))")
...
next
  ...
  proof -
    ...
    then have "Csem = (abs_stmt.Inhale ?P1;; fst (translate \<Delta> tys C1);; abs_stmt.Exhale (inhalify \<Delta> tys ?Q1)) \<or> Csem = (abs_stmt.Inhale ?P2;; fst (translate \<Delta> tys C2);; abs_stmt.Exhale (inhalify \<Delta> tys ?Q2))"
      using asm0 translate.simps(9)[of _ _ P1 C1 Q1 P2 C2 Q2]
      by (simp add: Let_def)


    moreover have "verifies_more (tcfe \<Delta> tys) (abs_stmt.Inhale ?P1;; fst (translate \<Delta> tys C1);; abs_stmt.Exhale (inhalify \<Delta> tys ?Q1)) (compile \<Delta> (tcfes tys) (stmt.Seq (stmt.Seq (stmt.Inhale P1) (fst (translate_syn C1))) (stmt.Exhale Q1)))"
      apply simp
      apply (rule verifies_more_seq)
      apply (rule verifies_more_seq)
          apply (rule verifies_more_inhale)
    ...
    moreover have "verifies_more (tcfe \<Delta> tys) (abs_stmt.Inhale ?P2;; fst (translate \<Delta> tys C2);; abs_stmt.Exhale (inhalify \<Delta> tys ?Q2))
...
next
  ...
  proof -
    ...
    have r: "Csem = abs_stmt.Inhale (?I \<inter> assertify_bexp b);; fst (translate \<Delta> tys C);; abs_stmt.Exhale (inhalify \<Delta> tys ?I)"
      using asm0 by simp

    have "verifies_more (tcfe \<Delta> tys) (abs_stmt.Inhale (?I \<inter> assertify_bexp b);; fst (translate \<Delta> tys C);; abs_stmt.Exhale (inhalify \<Delta> tys ?I))
