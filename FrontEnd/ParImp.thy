theory ParImp
  imports "../Viper/Instantiation" VHelper "../CoreIVL/SepLogic"
begin
...
text \<open>Part specific to non-Viper front-end\<close>
...
proof (induct rule: red.induct)
  ...
  then show ?case
    using store_typed_update[OF red_Assign(3), of x vints "VInt (edenot e s)"]
...
next
  ...
  then show ?case
    using store_typed_update[OF red_Alloc(4), of x vrefs "VRef (Address l)"]
    unfolding type_ctxt_store_def
...
next
  ...
  then show ?case
    using store_typed_update[OF red_Read(5), of x vints "VInt v"]
    unfolding type_ctxt_store_def
