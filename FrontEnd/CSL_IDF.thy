theory CSL_IDF
  imports ParImp "../CoreIVL/SepLogic" "../Viper/Instantiation"
begin
...
proof -
  have "wf_pre_virtual_state (\<lambda>l. if l \<in> dom h then 1 else 0, h)"
    by (smt (verit, best) gr_0_is_ppos domIff leD linorder_linear not_gr_0 wf_mask_simple_def wf_pre_virtual_stateI)
...
proof (rule virtual_state_ext)
  ...
  proof
    fix l show "get_vm \<omega> l = get_vm (mk_virtual_state (get_vh \<omega>)) l"
      by (metis gr_0_is_ppos assms(1) assms(2) binary_mask_def mk_virtual_state_charact(1) mk_virtual_state_simps(1) mk_virtual_state_simps(2) norm_preal(4) stable_virtual_state_def vstate_wf_imp)
  qed
qed (simp)
...
lemma wf_set_perm:
  ...
  by (smt (verit) gr_0_is_ppos assms(1) assms(2) fun_upd_apply get_vm_bound vstate_wf_imp wf_mask_simpleI wf_pre_virtual_stateI)
...
proof (rule stable_virtual_stateI)
  show "\<And>hla. get_vh (set_perm_and_value \<omega> hl p v) hla \<noteq> None \<Longrightarrow> PosReal.pnone < get_vm (set_perm_and_value \<omega> hl p v) hla"
    by (metis gr_0_is_ppos assms(1) assms(2) assms(3) fun_upd_apply stable_virtual_state_def update_perm_simps(1) update_perm_simps(2))
qed
...
lemma in_emp_then_empty_vheap:
  ...
  using assms unfolding emp_def apply simp
by (metis assms gr_0_is_ppos in_emp_then_zero_vmask not_gr_0 stabilize_is_stable
...
proof (rule CSL_I)
  ...
  = set_perm_and_value \<omega> (l, field_val) 1 (Some (VInt (edenot e s))) \<oplus> \<omega>f"
        using alloc_helper
        by (smt (verit, ccfv_threshold) gr_0_is_ppos EquiViper.add_masks_def Pair_inject \<open>sep_algebra_class.stable \<omega>\<close> asm0(1) asm0(2) asm0(3) binary_mask_and_stable_then_mk_virtual comm_monoid_add_class.add_0 leI mk_virtual_state_simps(1) option.discI order_less_imp_le padd_pos pperm_pgt_pnone stable_sum stable_virtual_state_def vstate_add_iff)
...
proof -
  ...
  then have "get_vh \<omega>f hl = None"
    using gr_0_is_ppos assms(2) pperm_pgt_pnone stable_virtual_state_def by blast
  show ?thesis
  proof (rule plus_virtual_stateI)
