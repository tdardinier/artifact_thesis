theory SyntacticAutomationHavoc
...
type_synonym 'a characterizer = "('a path_condition \<times> 'a substitution \<times> nat) list"

(* The natural number counts how many havocs have been encountered
\<longrightarrow> Every variable (in the store) is shifted by this amount
*)

(* \<sigma>' is \<sigma> with n additional values *)
definition shifted_by :: "nat \<Rightarrow> 'a npstate \<Rightarrow> 'a npstate \<Rightarrow> bool" where
  "shifted_by n \<sigma> \<sigma>' \<longleftrightarrow> (\<forall>k. \<sigma>' (n + k) = \<sigma> k)"

lemma shifted_by_trans:
  assumes "shifted_by n \<sigma> \<sigma>'"
      and "shifted_by m \<sigma>' \<sigma>''"
    shows "shifted_by (n + m) \<sigma> \<sigma>''"
  by (smt (verit, ccfv_SIG) add.commute assms(1,2) group_cancel.add1 shifted_by_def)
...
definition correct_characterizer :: "'a characterizer \<Rightarrow> (var, 'a) stmt \<Rightarrow> bool" where
  "correct_characterizer L C \<longleftrightarrow> (\<forall>\<sigma> \<sigma>'. (\<langle>C, \<sigma>\<rangle> \<rightarrow> \<sigma>') \<longleftrightarrow> (\<exists>(pc, subst, n) \<in> set L. \<exists>\<sigma>0. shifted_by n \<sigma> \<sigma>0 \<and> interp_pbexp pc \<sigma>0 \<and> \<sigma>' = apply_substitution subst \<sigma>0))"
(*
and \<exists>shift by n on right...
*)

lemma consequence_correct_characterizer:
  assumes "correct_characterizer L C"
      and "(pc, subst, n) \<in> set L"
      and "shifted_by n \<sigma> \<sigma>0"
      and "interp_pbexp pc \<sigma>0"
      and "\<sigma>' = apply_substitution subst \<sigma>0"
    shows "\<langle>C, \<sigma>\<rangle> \<rightarrow> \<sigma>'"
  using assms unfolding correct_characterizer_def by auto

lemma correct_characterizerI:
  assumes "\<And>\<sigma> \<sigma>'. \<langle>C, \<sigma>\<rangle> \<rightarrow> \<sigma>' \<Longrightarrow> \<exists>(pc, subst, n) \<in> set L. \<exists>\<sigma>0. shifted_by n \<sigma> \<sigma>0 \<and> interp_pbexp pc \<sigma>0 \<and> \<sigma>' = apply_substitution subst \<sigma>0"
      and "\<And>\<sigma> \<sigma>' pc subst \<sigma>0 n. (pc, subst, n) \<in> set L \<Longrightarrow> shifted_by n \<sigma> \<sigma>0 \<Longrightarrow> interp_pbexp pc \<sigma>0 \<Longrightarrow> \<sigma>' = apply_substitution subst \<sigma>0 \<Longrightarrow> \<langle>C, \<sigma>\<rangle> \<rightarrow> \<sigma>'"
...
lemma correct_characterizer_sem_charact:
  assumes "correct_characterizer L C"
  shows "sem C S = {(l, apply_substitution subst \<sigma>0) |l \<sigma> subst pc n \<sigma>0. (l, \<sigma>) \<in> S \<and> (pc, subst, n) \<in> set L \<and> interp_pbexp pc \<sigma>0 \<and> shifted_by n \<sigma> \<sigma>0}" (is "?A = ?B")
  unfolding sem_def
  apply rule
  apply rule
...
fun shift_map_and_update :: "(nat \<Rightarrow> ('a substitution \<times> nat \<times> nat)) \<Rightarrow> ('a substitution \<times> nat \<times> nat) \<Rightarrow> (nat \<Rightarrow> ('a substitution \<times> nat \<times> nat))" where
...
(* TODO: Adapt to new characterizer *)

fun forall_n :: "nat \<Rightarrow> 'a assertion \<Rightarrow> 'a assertion" where
  "forall_n 0 A = A"
| "forall_n (Suc n) A = AForall (forall_n n A)"


fun exists_n :: "nat \<Rightarrow> 'a assertion \<Rightarrow> 'a assertion" where
  "exists_n 0 A = A"
| "exists_n (Suc n) A = AExists (exists_n n A)"


fun replace_pvars_by_qvar_from_pexp_to_exp :: "nat \<Rightarrow> 'a pexp \<Rightarrow> 'a exp" where
  "replace_pvars_by_qvar_from_pexp_to_exp _ (PConst c) = EConst c"
| "replace_pvars_by_qvar_from_pexp_to_exp n (PVar m) = (if m < n then EQVar m else EPVar 0 (m - n))" (* Why 0? *)
| "replace_pvars_by_qvar_from_pexp_to_exp n (PFun f p) = EFun f (replace_pvars_by_qvar_from_pexp_to_exp n p)"
| "replace_pvars_by_qvar_from_pexp_to_exp n (PBinop p1 op p2) = EBinop (replace_pvars_by_qvar_from_pexp_to_exp n p1) op (replace_pvars_by_qvar_from_pexp_to_exp n p2)"

lemma shift_state_replace_pvars_same:
  assumes "shifted_by n \<sigma> \<sigma>0"
      and "\<And>x. x < n \<Longrightarrow> \<sigma>0 x = vals ! x"
  shows "interp_pexp p \<sigma>0 = interp_exp vals [(l, \<sigma>)] (replace_pvars_by_qvar_from_pexp_to_exp n p)"
  using assms
proof (induct p arbitrary: )
  case (PVar x)
  then show "interp_pexp (PVar x) \<sigma>0 = interp_exp vals [(l, \<sigma>)] (replace_pvars_by_qvar_from_pexp_to_exp n (PVar x))"
    apply simp using shifted_by_def[of n \<sigma> \<sigma>0]
...
fun transform_exp :: "(nat \<Rightarrow> 'a substitution \<times> nat \<times> nat) \<Rightarrow> 'a exp \<Rightarrow> 'a exp" where
  "transform_exp m (EPVar st x) = (let (subst, n, offset) = m st in
  if x < n then EQVar (offset + x) else pexp_to_exp st (subst x))"
...
| "transform_exp m e = e" (* LVar, QVar, Const *)
(*
Example!

transform_exp m (\<sigma>(x) + \<sigma>(y) = \<sigma>'(x) + \<sigma>'(y))
m \<sigma>' = ([x \<mapsto> a, y \<mapsto> 0], 1 symbol, 0 offset)
because of the 1 symbol, x is 1 and y is 2

m \<sigma> = ([x \<mapsto> 0, y \<mapsto> 1], 2 symbols, 1 offset
because of the 2 symbols, x is 2 and y is 3

Implicitly, this map means

\<forall><\<sigma>>. \<exists><\<sigma>'>. \<sigma>(x) + \<sigma>(y) = \<sigma>'(x) + \<sigma>'(y)
should become
\<forall><\<sigma>>. \<forall>2. \<forall>1. \<exists><\<sigma>'>. \<exists>0. 1 + 2 = \<sigma>'(a) + 0

e := (\<sigma>(x) + \<sigma>(y) = \<sigma>'(x) + \<sigma>'(y))
new exp: 1 + 2 = \<sigma>'(a) + 0

numbers mean fresh symbols
*)
...
(*
TODO: Think...
(* the map should take *two* nat, the first one says how many havocs, the second one says where its namespace of quantified values... *)
*)

fun transform_states_aux :: "nat \<Rightarrow> (nat \<Rightarrow> 'a substitution \<times> nat \<times> nat) \<Rightarrow> 'a list \<Rightarrow> 'a nstate list \<Rightarrow> 'a nstate list" where
  "transform_states_aux n m vals [] = []"
| "transform_states_aux n m vals (\<sigma> # l) = (fst \<sigma>, (\<lambda>x. let (subst, n, offset) = m n in
  if x < n then vals ! (offset + x) else interp_pexp (subst x) (snd \<sigma>))) # transform_states_aux (Suc n) m vals l"
...
lemma charact_transform_states_snd:
  assumes "st < length states"
      and "m (n + st) = (subst, n, offset)"
  shows "snd (transform_states_aux n m vals states ! st) x = interp_pexp (subst x) (snd (states ! st))"
  using assms
...
lemma size_shift_vars_exp[simp]:
  "size (shift_vars_exp n e) = size e"
  by (induct e) simp_all

lemma size_shift_vars[simp]:
  "size (shift_vars n A) = size A"
  by (induct A arbitrary: n) simp_all


fun transform_post_based_on_list :: "(nat \<Rightarrow> ('a substitution \<times> nat \<times> nat)) \<Rightarrow> 'a characterizer \<Rightarrow> 'a assertion \<Rightarrow> 'a assertion"
  where                                                                   
  "transform_post_based_on_list m L (AForallState A) = AForallState (foldr
  (\<lambda>(pc, subst, n) B. AAnd B (forall_n n (AImp (pbexp_to_assertion 0 pc) (transform_post_based_on_list (shift_map_and_update m (subst, n, 0)) L (shift_vars n A)))))
  L (AConst True))"
(*
maybe need to shift n times the values as well!
*)

| "transform_post_based_on_list m L (AExistsState A) = AExistsState (foldr
  (\<lambda>(pc, subst, n) B. AOr B (exists_n n (AAnd (pbexp_to_assertion 0 pc) (transform_post_based_on_list (shift_map_and_update m (subst, n, 0)) L (shift_vars n A)))))
  L (AConst False))"
