theory AbstractRefinesTotal
  imports "../Viper/Instantiation"
          "../Viper/EquiSemAuxLemma"
          "../CoreIVL/AbstractSemanticsProperties"
          TraceIndepProperty 
begin
...
lemma stable_dom_get_vh_eq_get_vm :
  assumes "stable st"
  shows "dom (get_vh st) = {hl. PosReal.ppos (get_vm st hl)}"
  using assms
  by (metis (mono_tags, lifting) Collect_cong dom_def norm_preal(1) stable_virtual_state_def vstate_wf_imp)
...
lemma get_valid_locs_def_ppos :
  "get_valid_locs \<omega> = {hl. PosReal.ppos (get_mh_total_full \<omega> hl)}"
  by (simp add:get_valid_locs_def norm_preal)

lemma eq_0_not_ppos :
  "p = 0 \<longleftrightarrow> \<not>PosReal.ppos p"
...
definition restrict_virtual_state :: "'a virtual_state \<Rightarrow> heap_loc set \<Rightarrow> 'a virtual_state"  (infixl "|`\<^sub>v"  110) where
  "st |`\<^sub>v A = Abs_virtual_state (get_vm st, get_vh st |` (A \<union> {hl. PosReal.ppos (get_vm st hl)}))"
...
lemma stabilize_vstate_restrict :
  "stabilize st = st |`\<^sub>v {hl. PosReal.ppos (get_vm st hl)}"
...
lemma stabilize_abs_state_restrict :
  "stabilize \<omega> = \<omega> |`\<^sub>a {hl. PosReal.ppos (get_vm (get_state \<omega>) hl)}"
...
lemma restrict_virtual_state_struct [simp] :
  "get_vh (st |`\<^sub>v A) = get_vh st |` (A \<union> {hl. PosReal.ppos (get_vm st hl)})"
  "get_vm (st |`\<^sub>v A) = get_vm st"
  apply (insert Rep_virtual_state[of st])
   apply (simp_all add:restrict_virtual_state_def get_vh_def get_vm_def wf_pre_virtual_state_def Abs_virtual_state_inverse)
   apply (simp add: Abs_virtual_state_inverse gr_0_is_ppos wf_pre_virtual_state_def)
  by (simp add: Abs_virtual_state_inverse gr_0_is_ppos wf_pre_virtual_state_def)
...
"\<lambda> st. (PosReal.pmin 1 \<circ> get_mh_total st, (Some \<circ> get_hh_total st))"
...
lemma t2a_virtual_state_get_vm[simp] :
  "get_vm (t2a_virtual_state \<phi>) = PosReal.pmin 1 \<circ> get_mh_total \<phi>"
...
lemma a2t_states_in_stable :
  ...
  shows "\<omega>\<^sub>t \<in> a2t_states ctxt \<omega> \<longleftrightarrow> \<omega> = stabilize (t2a_state \<omega>\<^sub>t) \<and> a2t_extend_ok ctxt \<omega> \<omega>\<^sub>t \<and>
    ...
    get_mp_total_full \<omega>\<^sub>t = zero_mask"
proof -
  from assms have Hvh: "dom (get_vh (get_state \<omega>)) = {hl. PosReal.ppos (get_vm (get_state \<omega>) hl)}"
...
lemma a2t_stateI [intro?] :
  ...
  subgoal
    ...
    apply (rule ext)
    apply (simp_all add:restrict_map_def a2t_extend_heap_def vstate_wf_ppos split:option.splits)
    by (meson domI domIff gr_0_is_ppos vstate_wf_ppos)
  subgoal
    ...
    using vstate_wf_Some
    using norm_preal(1) apply force
...
lemma ppos_get_mh_total_total_state_mask[simp] :
 "PosReal.ppos (get_mh_total (get_total_full (total_state_mask A)) hl) \<longleftrightarrow> hl \<in> A"
...
lemma red_pure_exp_cong_mh :
  assumes "\<And> hl. PosReal.ppos (get_mh_total (get_total_full \<omega>0) hl) = PosReal.ppos (get_mh_total (get_total_full \<omega>0') hl)"
  assumes "valid_a2t_exp e"
...
lemma red_pure_exp_cong_mh_eq :
  assumes "\<And> hl. PosReal.ppos (get_mh_total (get_total_full \<omega>0) hl) = PosReal.ppos (get_mh_total (get_total_full \<omega>) hl)"
  assumes "valid_a2t_exp e"
...
lemma ppos_pmin_1 :
  "PosReal.ppos (PosReal.pmin 1 p) \<longleftrightarrow> PosReal.ppos p"
  by (simp add:norm_preal preal_to_real)
...
lemma a2t_states_del_perm :
  ...
  subgoal for \<omega>\<^sub>t
    ...
    using gr_0_is_ppos apply fastforce
  ...
  subgoal for \<omega>\<^sub>t
    ...
    subgoal
      apply (rule ext; simp)
      by (metis (mono_tags, lifting) eq_0_not_ppos get_vm_bound inf.absorb2 minus_preal.abs_eq order_trans ppos_pmin_1 psub_smaller zero_preal_def)
    subgoal
     apply (rule ext; simp add:restrict_map_def ppos_pmin_1)
      using core_option.cases domIff get_hh_total_lookup_a2t_states preal_sub_ppos vstate_wf_ppos
      by (metis gr_0_is_ppos)
    subgoal
...
lemma red_exhale_set_AccWildcardI :
  ...
  assumes "get_vm (get_state \<omega>) (the_address r, f) \<noteq> 0"
...
lemma red_stmt_total_set_ExhaleI :
  ...
  shows "red_stmt_total_set ctxt R \<Lambda> (stmt.Exhale e) (a2t_states ctxt \<omega>) =
   Set.bind (red_exhale_set ctxt R (a2t_mask \<omega>) e (a2t_states ctxt \<omega>)) (\<lambda> \<omega>_exh.
       havoc_locs_state ctxt \<omega>_exh {loc. PosReal.ppos (get_vm (get_state \<omega>) loc) \<and>
          get_mh_total (get_total_full \<omega>_exh) loc = 0})"
...
lemma red_stmt_total_ok_FieldAssignE :
  ...
  shows  "\<exists> a v ty. declared_fields (program_total ctxt) f = Some ty \<and>
        ...
        get_vm (get_state \<omega>) (a,f) = 1 \<and>
        v \<in> sem_vtyp (absval_interp_total ctxt) ty"
proof -
  note calc = assms a2t_state_in_a2t_states[of ctxt \<omega>]
...
lemma red_stmt_total_set_FieldAssignI :
  ...
  assumes "get_vm (get_state \<omega>) (a,f) = 1"
  assumes "v \<in> sem_vtyp (absval_interp_total ctxt) ty"
  assumes "stable \<omega>"
  assumes "valid_a2t_stmt (stmt.FieldAssign e1 f e2)"
            ...
            subgoal for v
              ...
              subgoal
                ...
                apply (rule ext) subgoal for hl'
                  by (auto simp add:restrict_map_def norm_preal(1) dest:vstate_wf_Some)
                done
              subgoal
                apply (simp add:a2t_extend_ok_def abs_state_typing_def)
        ...
        subgoal for r ty p
          ...
          subgoal for a
            ...
            apply (rule exI[of _ "\<up>_"], safe)
              ...
              apply (simp add:add_set_asso[of "\<llangle>_\<rrangle>"])
              apply (rule bool_to_assertion_intro) apply (simp)
              apply (cut_tac vstate_wf_Some[of "get_state \<omega>" "(a, f)"]) prefer 2 apply (smt (verit, best) eq_0_not_ppos leD not_gr_0 positive_real_preal)
              apply (clarsimp)
        ...
        subgoal for r ty
          ...
          apply (rule exI[of _ "\<up>_"], safe)
            ...
            apply (cut_tac vstate_wf_Some[of "get_state \<omega>" "(the_address r, f)"])
             prefer 2 apply (smt (verit, best) eq_0_not_ppos leD not_gr_0 positive_real_preal)
            apply (clarsimp)
            apply (rule acc_heap_loc_starI)
...
lemma a2t_states_set_value :
  ...
  assumes "v \<in> sem_vtyp (absval_interp_total ctxt) ty"
  assumes "PosReal.ppos (get_vm (get_state \<omega>) hl)"
  ...
  apply (rule exI[of _ "x\<lparr>get_total_full := (get_total_full x)\<lparr>get_hh_total :=
    ...
    subgoal
      ...
      subgoal
        ...
        apply (rule ext; auto)
        using  vstate_wf_Some norm_preal(1) apply fastforce
...
lemma havoc_locs_state_a2t :
  ...
  shows "a2t_states ctxt (stabilize \<omega>) \<subseteq> Set.bind (a2t_states ctxt \<omega>) (\<lambda> \<omega>_exh.
         havoc_locs_state ctxt \<omega>_exh
                  {loc. loc \<in> dom (get_vh (get_state \<omega>)) \<and> \<not> PosReal.ppos (get_mh_total (get_total_full \<omega>_exh) loc)})" (is "_ \<subseteq> ?P")
proof (rule subsetI)
  ...
  from this assms(1) show ?case
    ...
    subgoal for \<omega>'
      ...
      apply (insert stable_dom_get_vh_eq_get_vm[of "get_state \<omega>"])
       apply (clarsimp simp add:stable_get_state eq_0_not_ppos)
      by force
    done
next case (Assert x)
  from this assms(1) show ?case
