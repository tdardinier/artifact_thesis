...
imports TotalSemantics TotalStateUtil
begin
...
proof -
  ...
  proof
    ...
    hence "PosReal.pgt (?m lh) 0"
      by (simp add: get_valid_locs_def)

    hence "PosReal.pgt (?m' lh) 0"
      using \<open>?m \<le> ?m'\<close>
      by (metis all_pos le_fun_def pos_perm_class.pgte_antisym preal_pnone_pgt)
...
proof 
  ...
  from this obtain p 
    where  "\<omega>1' = update_mh_loc_total_full \<omega>1 lh (PosReal.padd (get_mh_total_full \<omega>1 lh) p)"  and
           PermConstraint: "option_fold ((=) p) (p \<noteq> 0) p_opt" and
           AtMostWrite: "PosReal.pgte 1 (PosReal.padd (get_mh_total_full \<omega>1 lh) p)" and
           "R \<omega>1'"
    unfolding inhale_perm_single_def
    by blast

  let ?\<omega>0' = "update_mh_loc_total_full \<omega>0 lh (PosReal.padd (get_mh_total_full \<omega>0 lh) p)"    

  from \<open>\<omega>0 \<le> \<omega>1\<close> have "(get_mh_total_full \<omega>0 lh) \<le> (get_mh_total_full \<omega>1 lh)"
    using less_eq_full_total_stateD_2 
    by (auto dest: le_funD)

  hence *: "(PosReal.padd (get_mh_total_full \<omega>0 lh) p) \<le> (PosReal.padd (get_mh_total_full \<omega>1 lh) p)"
...
proof 
  ...
  from this obtain p 
    where  "\<omega>1' = update_mp_loc_total_full \<omega>1 lh (PosReal.padd (get_mp_total_full \<omega>1 lh) p)"  and
           PermConstraint: "option_fold ((=) p) (p \<noteq> 0) p_opt"
           "R \<omega>1'"
    unfolding inhale_perm_single_pred_def
    by blast

  let ?\<omega>0' = "update_mp_loc_total_full \<omega>0 lh (PosReal.padd (get_mp_total_full \<omega>0 lh) p)"    

  from \<open>\<omega>0 \<le> \<omega>1\<close> have "(get_mp_total_full \<omega>0 lh) \<le> (get_mp_total_full \<omega>1 lh)"
    using less_eq_full_total_stateD_2 
    by (auto dest: le_funD)

  hence *: "(PosReal.padd (get_mp_total_full \<omega>0 lh) p) \<le> (PosReal.padd (get_mp_total_full \<omega>1 lh) p)"
    by (simp add: PosReal.padd_mono)
...
proof (induction arbitrary: \<omega>')
  ...
  next
    case False
    with ExhAcc have SufficientPerm: "PosReal.pgte (mh (a, f)) (Abs_preal p)" and
...
next
  ...
  let ?p' = "(mh (a, f) - q)"
  from ExhAccWildcard have "mh (a, f) \<noteq> 0" and "\<omega>' = update_mh_total_full \<omega> (mh((a, f) := ?p'))"
    by (auto elim: exh_if_total.elims)

  with ExhAccWildcard have qprop: "0 < q \<and> mh (a, f) > q"
    by fastforce
  ...
  proof (subst \<open>\<omega>' = _\<close>, rule succ_full_total_stateI)
    ...
    show "get_mh_total_full \<omega> \<succeq> get_mh_total_full (update_mh_total_full \<omega> (mh((a, f) := ?p')))"      
      using mask_update_succ_aux_2[OF PosReal.pgt_implies_pgte, simplified PosReal.pgt_gt]
      unfolding \<open>mh = _\<close>
      by fastforce
  qed (simp_all add: succ_refl)
next
  case (ExhAccPred mp \<omega> e_args v_args e_p p pred_id)
  hence SufficientPerm: "PosReal.pgte (mp (pred_id, v_args)) (Abs_preal p)" and
...
next
  ...
  have *: "mp (pred_id, v_args) \<noteq> 0" and 
        "\<omega>' = update_mp_total_full \<omega> (mp((pred_id, v_args) := ?pnew))" and
        qprop:"0 < q \<and> q < mp (pred_id, v_args)"
    by (auto elim: exh_if_total.elims)
...
lemma plus_diff_full_total_state_upd_aux_1:
  assumes "\<omega>_inh \<oplus> (\<omega> \<ominus> \<omega>') = Some \<omega>_inh'"
    ...
    shows "\<omega>_inh' = update_mh_loc_total_full \<omega>_inh l (PosReal.padd (get_mh_total_full \<omega>_inh l) ((get_mh_total_full \<omega> l) - p))"
proof -
  let ?p' = "(PosReal.padd (get_mh_total_full \<omega>_inh l) ((get_mh_total_full \<omega> l) - p))"
...
lemma plus_diff_full_total_state_upd_aux_2:
  assumes "\<omega>_inh \<oplus> (\<omega> \<ominus> \<omega>') = Some \<omega>_inh'"
      ...
      and "\<omega> \<succeq> \<omega>'"
    shows "\<omega>_inh' = update_mp_loc_total_full \<omega>_inh l (PosReal.padd (get_mp_total_full \<omega>_inh l) ((get_mp_total_full \<omega> l) - p))"
proof -
  let ?p' = "(PosReal.padd (get_mp_total_full \<omega>_inh l) ((get_mp_total_full \<omega> l) - p))"
  from \<open>\<omega>_inh \<oplus> (\<omega> \<ominus> \<omega>') = Some \<omega>_inh'\<close>
  have "\<omega>_inh' = update_m_total_full \<omega>_inh (add_masks (get_mh_total_full \<omega>_inh) (get_mh_total_full (\<omega> \<ominus> \<omega>')))
                                           (add_masks (get_mp_total_full \<omega>_inh) (get_mp_total_full (\<omega> \<ominus> \<omega>')))"
    using plus_Some_full_total_state_eq
...
proof (induction arbitrary: \<omega>_inh \<omega>_inh' \<omega>')
  ...
  next
    ...
    hence PermConditions: "0 \<le> p \<and> PosReal.pgte (mh (a, f)) (Abs_preal p)" and
                          "\<omega>' = update_mh_loc_total_full \<omega> (a, f) (mh (a, f) - Abs_preal p)"
      using \<open>r = Address a\<close> ExhAcc
      by (auto elim: exh_if_total.elims)

    let ?loc = "(a,f)"
    let ?p' = "PosReal.padd (get_mh_total_full \<omega>_inh (a,f)) (Abs_preal p)"
    have "\<omega>_inh' = update_mh_loc_total_full \<omega>_inh ?loc ?p'" (is "_ = ?upd_\<omega>_inh")        
    proof -
      let ?mh_af = "(get_mh_total_full \<omega> (a, f))"
      have "\<omega>_inh' = update_mh_loc_total_full \<omega>_inh ?loc (PosReal.padd (get_mh_total_full \<omega>_inh ?loc) (?mh_af - (?mh_af - Abs_preal p)))"
    ...
    hence "get_mh_total_full \<omega>_inh' ?loc = PosReal.padd (get_mh_total_full \<omega>_inh ?loc) (Abs_preal p)"
      by simp

    hence PermConstraint': "PosReal.pgte 1 (PosReal.padd (get_mh_total_full \<omega>_inh ?loc) (Abs_preal p))"
...
next
  ...
  from ExhAccWildcard have "mh ?loc \<noteq> 0" and "\<omega>' = update_mh_loc_total_full \<omega> ?loc ?p_exh_new"
    by (auto elim: exh_if_total.elims)
  ...
  show ?case
  proof -
    let ?p' = "(PosReal.padd (get_mh_total_full \<omega>_inh ?loc) (get_mh_total_full \<omega> ?loc - ?p_exh_new))"
    have "\<omega>_inh' = update_mh_loc_total_full \<omega>_inh ?loc ?p'" (is "_ = ?upd_\<omega>_inh")    
      using plus_diff_full_total_state_upd_aux_1[OF \<open>\<omega>_inh \<oplus> (\<omega> \<ominus> \<omega>') = Some \<omega>_inh'\<close> \<open>\<omega>' = _\<close> \<open>\<omega> \<succeq> \<omega>'\<close>]
      by blast
...
next
  ...
  proof -
    ...
    have PermConditions: "0 \<le> p \<and> PosReal.pgte (mp ?loc) (Abs_preal p)" and
                         "\<omega>' = update_mp_loc_total_full \<omega> (pred_id, v_args) (mp ?loc - Abs_preal p)"
      using ExhAccPred
      by (auto elim: exh_if_total.elims)

    let ?p' = "PosReal.padd (get_mp_total_full \<omega>_inh ?loc) (Abs_preal p)"
    have "\<omega>_inh' = update_mp_loc_total_full \<omega>_inh ?loc ?p'" (is "_ = ?upd_\<omega>_inh")        
    proof -
      let ?mp_loc = "(get_mp_total_full \<omega> ?loc)"
      have "\<omega>_inh' = update_mp_loc_total_full \<omega>_inh ?loc (PosReal.padd (get_mp_total_full \<omega>_inh ?loc) (?mp_loc - (?mp_loc - Abs_preal p)))"
    ...
    hence "get_mp_total_full \<omega>_inh' ?loc = PosReal.padd (get_mp_total_full \<omega>_inh ?loc) (Abs_preal p)"
      by simp
...
next
  ...
  proof -
    let ?loc = "(pred_id, v_args)"
    let ?pnew = "mp (pred_id, v_args) - q"
    let ?p' = "(PosReal.padd (get_mp_total_full \<omega>_inh ?loc) (get_mp_total_full \<omega> ?loc - ?pnew))"

    have "mp ?loc \<noteq> 0" and 
    ...
    hence "get_mp_total_full \<omega> ?loc - ?pnew \<noteq> 0"
      unfolding \<open>mp = _\<close>
      using qprop
    ...
    have "\<omega>_inh' \<in> ?W"
      unfolding inhale_perm_single_pred_def
      using \<open>StateCons \<omega>_inh'\<close> 
            \<open>get_mp_total_full \<omega> ?loc - ?pnew \<noteq> 0\<close>             
            \<open>\<omega>_inh' = _\<close> 
      by auto     
...
proof -
  have SufficientPerm: "PosReal.pgte 1 (PosReal.padd (get_mh_total_full \<omega> lh) p)"
    using InhPermSingle1
    unfolding inhale_perm_single_def
    by fastforce

  let ?\<omega>0 = "(update_mh_loc_total_full \<omega> lh (PosReal.padd (get_mh_total_full \<omega> lh) p))"
  ...
  let ?\<omega>1 = "update_mh_loc_total_full \<omega>' lh (PosReal.padd (get_mh_total_full \<omega>' lh) p)"
  ...
  next
    show "option_fold ((=) p) (p \<noteq> 0) (Some p)"
      by simp
  next
    show "PosReal.pgte 1 (PosReal.padd (get_mh_total_full \<omega>' lh) p)"
    ...
    proof (rule th_result_rel_convert)
      ...
      proof (cases r)
        ...
        with \<open>\<omega>Elem \<in> W'\<close> inhale_perm_single_nonempty have 
         "\<omega>Elem = update_mh_loc_total_full \<omega> (a,f) (PosReal.padd (get_mh_total_full \<omega> (a,f)) (Abs_preal p))"
          unfolding \<open>W' = _\<close>
          by fastforce
        ...
        proof (simp)
          ...
          proof (rule inhale_perm_single_elem)
            show "\<omega>Elem\<lparr>get_store_total := get_store_total \<omega>2\<rparr> = update_mh_loc_total_full \<omega>2 (a, f) (PosReal.padd (get_mh_total_full \<omega>2 (a, f)) ((Abs_preal p)))"
              unfolding \<open>\<omega>Elem = _\<close>
            proof -
              have "update_mh_loc_total_full \<omega> (a, f) (PosReal.padd (get_mh_total_full \<omega> (a, f)) (Abs_preal p))\<lparr>get_store_total := get_store_total \<omega>2\<rparr> = 
                    update_mh_loc_total_full (\<omega>\<lparr>get_store_total := get_store_total \<omega>2\<rparr>) (a,f) (PosReal.padd (get_mh_total_full \<omega> (a, f)) (Abs_preal p))"
                by force
              ...
              ultimately show 
                "update_mh_loc_total_full \<omega> (a, f) (PosReal.padd (get_mh_total_full \<omega> (a, f)) (Abs_preal p))\<lparr>get_store_total := get_store_total \<omega>2\<rparr> =
                 update_mh_loc_total_full \<omega>2 (a, f) (PosReal.padd (get_mh_total_full \<omega>2 (a, f)) (Abs_preal p))"
                using InhAcc
                by force
            qed
          next
            ...
            qed
          next
            show "PosReal.pgte 1 (PosReal.padd (get_mh_total_full \<omega>2 (a, f)) (Abs_preal p))"
            proof -
              from \<open>\<omega>Elem \<in> _\<close>
              have "PosReal.pgte 1 (PosReal.padd (get_mh_total_full \<omega> (a, f)) (Abs_preal p))"
                unfolding \<open>W' = _\<close> inhale_perm_single_def \<open>r = _\<close>
                by simp
              thus ?thesis
                using InhAcc
...
lemma red_exhale_accI:
  assumes "ctxt, R, (Some \<omega>0) \<turnstile> \<langle>e_r; \<omega>\<rangle> [\<Down>]\<^sub>t Val (VRef r)"
      ...
      and "res = exh_if_total (p \<ge> 0 \<and> (if r = Null then p = 0 else PosReal.pgte (get_mh_total_full \<omega> (a,f)) (Abs_preal p))) \<omega>'" 
...
proof (induction arbitrary: \<omega>2 res2)
  ...
  proof (rule red_exhale_accI[OF RedRef RedPerm], simp, simp)
    show "res2 = exh_if_total (0 \<le> p \<and> (if r = Null then p = 0 else PosReal.pgte (get_mh_total_full \<omega>2 (the_address r, f)) (Abs_preal p))) ?\<omega>'"
    proof (cases r)
      case (Address a')
      hence "r = Address a"
        using ExhAcc
