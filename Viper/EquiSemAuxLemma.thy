theory EquiSemAuxLemma
  imports EquiViper PredicatesUtil ViperUtil
begin
...
subsection \<open>Properties of Abstract Separation Algebra\<close>
(*
...
proof -
  ...
  proof (rule wf_pre_virtual_stateI)
    ...
    then have "\<not>\<pi> hl > 0"
      using gr_0_is_ppos by auto
    then show "\<pi> hl > 0 \<Longrightarrow> h hl \<noteq> None"
      by simp
  next
...
proof -
  ...
  proof (rule wf_pre_virtual_stateI)
    ...
    then have "?xm hl \<ge> ?ym hl"
      by (simp add: pos_perm_class.sum_larger)
    moreover have "?ym hl > 0" using \<open>0 < ?ym hl\<close> apply transfer
      by simp
    ultimately have "?xm hl > 0"
      by simp
    then have "PosReal.ppos (?xm hl)" apply transfer
      by simp
    moreover have "x = (?xm, ?xh)"
      by simp
    ultimately obtain v where "?xh hl = Some v"
      by (meson \<open>0 < fst x hl\<close> assms(1) not_None_eq wf_pre_virtual_state_def)
...
lemma greater_heap_rule:
  ...
  shows "b \<succeq> a"
  using assms commutative greaterI greater_def option.exhaust plus_option.simps(2) succ_refl
...
definition stable_virtual_state :: "'v virtual_state \<Rightarrow> bool" where
  "stable_virtual_state x \<longleftrightarrow> (\<forall>hl :: heap_loc. get_vh x hl \<noteq> None \<longrightarrow> PosReal.ppos (get_vm x hl))"

definition stabilize2pre :: "'v virtual_state \<Rightarrow> 'v pre_virtual_state" where
  "stabilize2pre x = (get_vm x, get_vh x |` {hl. PosReal.ppos (get_vm x hl)})"
...
proof -
  obtain \<pi> h where "stabilize2pre x = (\<pi>, h)" "\<pi> = get_vm x" "h = get_vh x |` {hl. PosReal.ppos (get_vm x hl)}"
  ...
  proof (rule wf_pre_virtual_stateI)
    ...
    proof -
      ...
      then have "hx hl \<noteq> None"
        by (metis \<open>\<pi> hl > 0\<close> \<open>Rep_virtual_state x = (\<pi>x, hx)\<close> calculation(2) get_vh_def gr_0_is_ppos prod.sel(2) vstate_wf_imp)
      moreover have "get_vh x = hx" using \<open>Rep_virtual_state x = (\<pi>x, hx)\<close>
        by (simp add:get_vh_def)
      ultimately show ?thesis
        by simp
    qed
    moreover have "0 < (get_vm x hl)"
      using \<open>\<pi> hl > 0\<close> \<open>\<pi> = get_vm x\<close> by blast
    ultimately show "h hl \<noteq> None"
      by (simp add: \<open>h = get_vh x |` {hl. PosReal.ppos (get_vm x hl)}\<close> norm_preal(1))
  next
    show "wf_mask_simple \<pi>"
...
lemma vstate_stabilize_structure:
  shows "get_vm (stabilize x) = get_vm x"
    and "get_vh (stabilize x) = get_vh x |` {hl. PosReal.ppos (get_vm x hl)}"
proof -
  ...
  show "get_vh (stabilize x) = get_vh x |` {hl. PosReal.ppos (get_vm x hl)}"
...
instance proof
  ...
  show "sep_algebra_class.stable (stabilize x)"
    by (simp add: vstate_stabilize_structure(1) vstate_stabilize_structure(2) pperm_pnone_pgt stable_virtual_state_def restrict_map_def)
  show "sep_algebra_class.stable x \<Longrightarrow> stabilize x = x"
    apply (rule virtual_state_ext)
     apply (simp_all add: vstate_stabilize_structure stable_virtual_state_def)
    apply (rule ext)
  ...
  proof (rule plus_virtual_stateI)
    ...
    proof (rule plus_funI)
      fix l show "Some (get_vh x l) = get_vh (stabilize x) l \<oplus> get_vh |x| l"
        ...
        apply (metis gr_0_is_ppos vstate_stabilize_structure(1) \<open>sep_algebra_class.stable (stabilize x)\<close> core_is_pure core_option.simps(1) core_structure(2) stable_virtual_state_def)
        apply (simp add: vstate_stabilize_structure(2) core_structure(2) plus_val_id restrict_map_def)
        by (metis gr_0_is_ppos \<open>sep_algebra_class.stable (stabilize x)\<close> commutative core_structure(2) plus_option.simps(2) stable_virtual_state_def vstate_stabilize_structure(1))
    qed
    show "Some (get_vm x) = get_vm (stabilize x) \<oplus> get_vm |x|"
      by (simp add: vstate_stabilize_structure(1) core_structure(1) zero_mask_identity)
  qed


  show "Some x = a \<oplus> b \<Longrightarrow> Some (stabilize x) = stabilize a \<oplus> stabilize b"
    apply (clarsimp simp add: vstate_add_iff vstate_stabilize_structure restrict_map_def)
    apply (rule plus_funI)
    by (smt (verit) gr_0_is_ppos EquiViper.add_masks_def PosReal.ppos.rep_eq commutative core_is_pure core_option.simps(1) plus_funE plus_preal.rep_eq pperm_pnone_pgt val_option_sum vstate_wf_ppos)

  show "Some x = a \<oplus> stabilize |b| \<Longrightarrow> x = a"
    apply (clarsimp simp add: vstate_add_iff vstate_stabilize_structure
           core_structure ValueAndBasicState.zero_mask_def)
  proof -
...
lift_definition add_perm :: "'a virtual_state \<Rightarrow> heap_loc \<Rightarrow> preal \<Rightarrow> 'a val \<Rightarrow> 'a virtual_state" is
  "\<lambda> st hl p v. ((get_vm st)(hl := PosReal.pmin 1 (get_vm st hl + p)), (get_vh st)(hl \<mapsto> v))"
...
lemma add_perm_get_vm [simp] :
  "get_vm (add_perm st hl p v) = (get_vm st)(hl := PosReal.pmin 1 (get_vm st hl + p))"
...
lift_definition del_perm :: "'a virtual_state \<Rightarrow> heap_loc \<Rightarrow> preal \<Rightarrow> 'a virtual_state" is
  ...
  by (smt (verit) diff_0_right less_eq_preal.rep_eq less_preal.rep_eq not_Some_eq not_gr_0 one_preal.rep_eq
      option.inject order_less_imp_not_less vstate_wf_imp zero_preal.rep_eq)
...
"\<lambda> hl p v. ((\<lambda> hl'. if hl = hl' then (PosReal.pmin 1 p) else 0), [hl \<mapsto> v])"
...
lemma acc_virt_get_vm [simp]:
  shows "get_vm (acc_virt hl p v) hl' = (if hl = hl' then PosReal.pmin 1 p else 0)"
  by (simp add:get_vm_def acc_virt.rep_eq)

lemma acc_virt_get_vm' :
  shows "get_vm (acc_virt hl p v) = (\<lambda> hl'. (if hl = hl' then PosReal.pmin 1 p else 0))"
...
lemma acc_virt_plus :
  shows "Some st2 = st1 \<oplus> acc_virt hl p v \<longleftrightarrow> 
    st2 = add_perm st1 hl p v \<and> get_vh st1 ## [hl \<mapsto> v] \<and> get_vm st1 hl + PosReal.pmin 1 p \<le> 1"
