theory EquiViper
  imports Main "../CoreIVL/AbstractSemantics"
    ValueAndBasicState PartialMap Binop DeBruijn
begin
...
definition wf_pre_virtual_state :: "'a pre_virtual_state \<Rightarrow> bool" where
  "wf_pre_virtual_state st \<longleftrightarrow> (\<forall>hl. fst st hl > 0 \<longrightarrow> snd st hl \<noteq> None) \<and> wf_mask_simple (fst st)"
...
proof (rule wf_pre_virtual_stateI)
  show "\<And>hl. zero_mask hl > 0 \<Longrightarrow> empty_heap hl \<noteq> None"
...
proof clarify
  fix a aa ab :: "preal mask"
  fix b ba bb :: "'a partial_heap"
  fix ac bc
  assume asm0: "\<forall>hl. fst (a, b) hl > 0 \<longrightarrow> (\<exists>y. snd (a, b) hl = Some y)"
                "\<forall>hl. fst (aa, ba) hl > 0 \<longrightarrow> (\<exists>y. snd (aa, ba) hl = Some y)"
                "(a, b) \<oplus> (aa, ba) = Some (ab, bb)" "fst (ab, bb) (ac, bc) > 0"
  then have "a (ac, bc) > 0 \<or> aa (ac, bc) > 0"
    using plus_prodE preal_plusE
    by (metis (mono_tags, lifting) add.right_neutral fst_conv plus_funE pperm_pnone_pgt)
