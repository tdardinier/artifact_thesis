theory Instantiation
  imports "../CoreIVL/AbstractSemanticsProperties" EquiViper EquiSemAuxLemma
begin
...
proof -
  ...
  proof (rule wf_pre_virtual_stateI)
    fix hl assume asm0: "0 <  (((get_vm (get_state \<omega>))(l := PosReal.pnone)) hl)"
    ...
    next
      case False
      then have "0 <  (get_vm (get_state \<omega>) hl)"
        using asm0 by force
      then show ?thesis
  ...
  proof (rule virtual_state_ext)
    ...
    proof
      fix x show "get_h (stabilize (remove_only \<omega> l)) x = get_h (remove_only (stabilize \<omega>) l) x"
        apply (cases "x = l")
        apply (metis \<open>get_m (stabilize (remove_only \<omega> l)) = get_m (remove_only (stabilize \<omega>) l)\<close> fun_upd_same gr_0_is_ppos remove_only_charact(1)
            stabilize_is_stable stable_get_state stable_virtual_state_def vstate_wf_ppos)
        apply (cases "get_h (stabilize (remove_only \<omega> l)) x")
         apply (metis fun_upd_apply get_state_stabilize gr_0_is_ppos remove_only_charact(1,2) stabilize_is_stable stable_virtual_state_def
            vstate_stabilize_structure(1) vstate_wf_ppos)
      proof -
        fix a :: "'a val"
        ...
        then show ?thesis
          using a2 a1
          by (smt (z3) \<open>get_m (stabilize (remove_only \<omega> l)) = get_m (remove_only (stabilize \<omega>) l)\<close> fun_upd_apply get_vh_Some_greater gr_0_is_ppos
              remove_only_charact(1) stable_get_state stable_virtual_state_def vstate_wf_Some)
      qed        
    qed
  qed
...
inductive SL_Custom :: "('a val, (field_ident \<rightharpoonup> 'a val set)) abs_type_context \<Rightarrow> 'a equi_state set \<Rightarrow> 'a custom \<Rightarrow> ('a equi_state set) \<Rightarrow> bool"
  where
...
proof (rule virtual_state_ext)
  ...
  proof (rule ext)
    fix x show "get_vh a x = get_vh b x"
      by (metis assms(1,2,3,4) gr_0_is_ppos stable_virtual_state_def vstate_wf_Some)
  qed
...
lemma stabilize_set_value:
  ...
  apply (metis gr_0_is_ppos assms fun_upd_apply get_vh_vm_set_value(1) get_vh_vm_set_value(2) get_vm_stabilize stabilize_is_stable stable_virtual_state_def)
...
lemma concrete_post_LocalAssign :
  ...
  shows "concrete_red_stmt_post \<Delta> (AbstractSemantics.LocalAssign x e) \<omega> S"
  using assms
...
lemma concrete_post_Havoc :
  ...
  shows "concrete_red_stmt_post \<Delta> (AbstractSemantics.Havoc x) \<omega> S"
  using assms
  apply (simp add: concrete_red_stmt_post_def)
  apply (rule exI, rule conjI)
...
lemma concrete_post_Skip :
  shows "\<omega> \<in> S \<Longrightarrow> concrete_red_stmt_post \<Delta> AbstractSemantics.Skip \<omega> S"
  unfolding concrete_red_stmt_post_def
  apply (rule exI, rule conjI)
